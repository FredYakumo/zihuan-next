...
`WeaviateRef::upsert_image_record(...)` accepts a `PersistedMedia` and a vector. Weaviate object properties are derived directly from the `PersistedMedia` struct, keeping image search aligned with the same media facts used by QQ messages, multimodal builders, and outbound sending.
