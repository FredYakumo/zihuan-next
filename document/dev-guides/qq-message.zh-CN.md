...
从最终用户视角看，当前在线 QQ Chat Agent 支持“插嘴式打断”。

具体含义是：

- 如果 Agent 还没有发出最终回复，而同一个发送者又发来了一条新消息，这条消息会被视为 steer，而不是“忙碌冲突”。
- 如果 Agent 在 tool call 之后还会继续进入下一轮 Brain 推理，这些 steer 会在下一次推理前被吸收进当前尚未完成的思考过程；当同一窗口内积累了多条 steer 时，它们会按到达顺序拼成一条显式用户消息，而不是逐条分别送给模型。
- 如果当前回复流程已经来不及吸收这条新消息，那么这条 steer 会在当前流程结束后自动触发下一轮 follow-up 对话。
- steer 会作为一条新的显式用户消息暴露给模型，而不是隐藏的 system 指令。
- 单次活跃回复流程里最多接受多少条 steer，由 `QqChatAgentConfig.max_steer_count` 控制；当前默认值是 `4`。

实际使用说明：

- 如果用户想纠正、缩小范围，或临时改变一个尚未完成的回答方向，可以直接继续发消息。
- 如果用户在 Agent 正忙时连续补发多条短消息，而当前 Brain 后续刚好还有下一轮 inference，那么模型通常会在那一轮里一次性看到合并后的这串补充内容。
- 这种合并不是“先等用户发完再开始首轮思考”的静默窗口机制；首条消息仍会立即启动当前回复流程。
- 如果用户在同一条尚未结束的回复流程里连续发送过多重叠消息，超过上限之后，后续 steer 可能会被丢弃。
- 群聊中的初始触发条件没有变化：是否进入 QQ Chat 回复流程，仍然遵循当前的群聊 `@` 规则。
