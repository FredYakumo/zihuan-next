...
```
zihuan-next_aibot-800b/
├── src/
│   ├── main.rs                    # Entry point, CLI parsing, mode selection
│   ├── config.rs                  # YAML config loader (file -> env -> defaults)
│   ├── error.rs                   # Unified error types
│   │
│   ├── node/                      # Node Graph Engine
│   │   ├── mod.rs                 # Node trait, NodeGraph, execution logic
│   │   ├── data_value.rs          # DataType/DataValue enums
│   │   ├── graph_io.rs            # JSON serialization (serde)
│   │   ├── registry.rs            # Node type registry & factory
│   │   ├── util_nodes.rs          # Simple utility nodes
│   │   ├── database_nodes.rs      # Redis/MySQL config nodes
│   │   └── message_nodes.rs       # Message storage nodes
│   │
│   ├── bot_adapter/               # Bot Integration Layer
│   │   ├── adapter.rs             # WebSocket client, event loop
│   │   ├── event.rs               # Event dispatcher & handler registry
│   │   ├── node_impl.rs           # BotAdapterNode, MessageSenderNode
│   │   ├── message_event_to_string.rs  # Event -> String converter node
│   │   └── models/                # Message event data structures
│   │       ├── event_model.rs     # MessageEvent, MessageType enums
│   │       ├── message.rs         # Parsed message types
│   │       └── profile.rs         # User/group profile structs
│   │
│   ├── llm/                       # LLM Integration Layer
│   │   ├── llm_api.rs             # HTTP client for chat endpoints
│   │   ├── node_impl.rs           # LLMNode, AgentNode, TextProcessorNode
│   │   ├── agent/
│   │   │   └── brain.rs           # BrainAgent (multi-tool orchestrator)
│   │   ├── function_tools/        # Tool implementations
│   │   │   ├── math.rs            # Math calculator
│   │   │   ├── chat_history.rs    # Context retrieval
│   │   │   └── code_writer.rs     # Code generation
│   │   └── prompt/                # Prompt templates
│   │       └── brain.rs           # System prompts
│   │
│   ├── util/                      # Utility Modules
│   │   ├── message_store.rs       # Three-tier message storage
│   │   └── url_utils.rs           # URL parsing helpers
│   │
│   └── ui/                        # GUI Layer (Slint)
│       ├── node_graph_view.rs     # Main UI logic, callbacks
│       ├── graph_window.slint     # UI component definitions
│       ├── selection.rs           # Node selection state
│       ├── window_state.rs        # Persistent window config
│       └── node_render/           # Custom node renderers
│           ├── preview_string.rs  # Preview node special rendering
│           └── string_data.rs     # String input node rendering
│
├── database/                      # Python/SQLAlchemy Models
│   └── models/
│       └── message_record.py      # Message persistence schema
│
├── migrations/                    # Alembic Database Migrations
│   └── versions/
│
├── document/                      # Documentation
│   ├── node-graph-json.md         # JSON format spec
│   ├── node-development.md        # Node creation guide
