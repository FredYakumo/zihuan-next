...
2. **MySQL**：用于长期消息持久化。
    ```bash
    # 使用 Docker 启动 Redis 和 MySQL（最简单的方式）
...
应用需要一个 `config.yaml` 文件。

1. 复制示例配置：
    ```bash
    cp config.yaml.example config.yaml
    ```
2. 编辑 `config.yaml`，填写你的具体值：
    - **BOT_SERVER_URL**：你的 QQ 机器人 WebSocket 接口地址。
    - **TOKEN**：认证令牌。
    - **REDIS_URL / MYSQL_URL**：数据库连接字符串。
    - **LLM_API_BASE**：你的 LLM 提供商 URL（例如 OpenAI、本地 LLM）。
