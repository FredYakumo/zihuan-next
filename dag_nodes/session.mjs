...
export const nodes = [
  {
    ...
    execute: async ({ inputs, zihuan }) => zihuan.session.get(inputs.session_ref, inputs.sender_id),
  },
  {
    ...
    execute: async ({ inputs, zihuan }) => ({ cleared: await zihuan.session.clear(inputs.session_ref, inputs.sender_id) }),
  },
  {
    ...
    execute: async ({ inputs, zihuan }) => zihuan.session.tryClaim(inputs.session_ref, inputs.sender_id, inputs.state_json),
  },
  {
    ...
    execute: async ({ inputs, zihuan }) => ({ released: await zihuan.session.release(inputs.session_ref, inputs.sender_id) }),
  },
];
