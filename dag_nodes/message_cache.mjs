...
export const nodes = [
  {
    ...
    execute: async ({ inputs, sdk }) => ({ success: await sdk.messageCache.append(inputs.cache_ref, inputs.sender_id, inputs.messages) }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => ({ messages: await sdk.messageCache.get(inputs.cache_ref, inputs.sender_id, inputs.fallback) }),
  },
  {
    ...
    input_ports: [port("cache_ref", "LLMMessageSessionCacheRef"), port("sender_id", "String"), port("messages", { Vec: "LLMMessage" })], output_ports: [port("success", "Boolean")],
    execute: async ({ inputs, sdk }) => ({ success: await sdk.messageCache.set(inputs.cache_ref, inputs.sender_id, inputs.messages) }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => ({ cleared: await sdk.messageCache.clear(inputs.cache_ref, inputs.sender_id) }),
  },
];
