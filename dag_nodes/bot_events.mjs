...
export const nodes = [
  {
    ...
    execute: async ({ inline_values, sdk }) => ({ ims_bot_adapter: await sdk.bot.adapter(inline_values.config_id ?? inline_values.connection_id) }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => ({ result: await sdk.bot.senderFromEvent(inputs.message_event) }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => ({ result: await sdk.bot.senderIdFromEvent(inputs.message_event) }),
  },
  {
    type_id: "extract_group_id_from_event", display_name: "提取群号", category: "Bot适配器", description: "从群消息事件中提取群号（字符串）",
    input_ports: [port("message_event", "MessageEvent")], output_ports: [port("result", "String")],
    execute: async ({ inputs, sdk }) => ({ result: await sdk.bot.groupIdFromEvent(inputs.message_event) }),
  },
  {
    type_id: "extract_optional_group_id_from_event", display_name: "提取可选群号", category: "Bot适配器", description: "从消息事件中提取群号；私聊时返回空字符串",
    input_ports: [port("message_event", "MessageEvent")], output_ports: [port("result", "String")],
    execute: async ({ inputs, sdk }) => ({ result: await sdk.bot.optionalGroupIdFromEvent(inputs.message_event) }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => ({ message_list: await sdk.bot.messagesFromEvent(inputs.message_event) }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => sdk.bot.filterEventType(inputs.message_event, inputs.filter_type),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => sdk.bot.send(sdk.resources.botAdapter(inputs.ims_bot_adapter), inputs.sender, inputs.message),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => sdk.bot.sendBatches(sdk.resources.botAdapter(inputs.ims_bot_adapter), inputs.target_id, inputs.message_batches, { target_type: "friend", delay_millis: inputs.delay_millis }),
  },
  {
    ...
    input_ports: [port("ims_bot_adapter", "BotAdapterRef"), port("target_id", "String"), port("message_batches", { Vec: { Vec: "QQMessage" } }), port("delay_millis", "Integer", { required: false })],
    output_ports: [port("success", "Boolean"), port("summary", "String"), port("message_ids", { Vec: "Integer" })],
    execute: async ({ inputs, sdk }) => sdk.bot.sendBatches(sdk.resources.botAdapter(inputs.ims_bot_adapter), inputs.target_id, inputs.message_batches, { target_type: "group", delay_millis: inputs.delay_millis }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => sdk.bot.sendBatches(sdk.resources.botAdapter(inputs.ims_bot_adapter_ref), inputs.target_id, inputs.message_batches, { target_type: inputs.target_type }),
  },
  {
    ...
    execute: async ({ inputs, sdk }) => sdk.bot.extractMessages(sdk.resources.botAdapter(inputs.ims_bot_adapter), inputs.message_event, { message_id: inputs.message_id, s3_ref: inputs.s3_ref }),
  },
];
