...
/// Filters a MessageEvent by its type (private / group) and routes it to
/// the matching output port (`true_event`) or the non-matching port (`false_event`).
...
impl Node for ExtractSenderIdFromEventNode {
    fn id(&self) -> &str { &self.id }
    fn name(&self) -> &str { &self.name }
    fn description(&self) -> Option<&str> { Some("从消息事件中提取发送者的QQ号（字符串）") }
    ...
    node_output![
        port! { name = "result", ty = String, desc = "发送者的QQ号字符串" },
    ];
    ...
    fn execute(&mut self, inputs: HashMap<String, DataValue>) -> Result<HashMap<String, DataValue>> {
        ...
        let mut outputs = HashMap::new();
        outputs.insert("result".to_string(), DataValue::String(event.sender.user_id.to_string()));
        Ok(outputs)
    }
}
