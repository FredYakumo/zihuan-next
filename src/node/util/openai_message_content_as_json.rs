...
impl Node for OpenAIMessageContentAsJsonNode {
    ...
    node_output![port! { name = "json", ty = Json, desc = "由 OpenAIMessage.content 解析得到的 JSON" },];
    ...
    fn execute(
        ...
        let json = serde_json::from_str(content).map_err(|err| {
            Error::ValidationError(format!("OpenAIMessage content is not valid JSON: {}", err))
        })?;
