...
use zihuan_graph_engine::function_graph::{embedded_function_config_from_node, FUNCTION_CONFIG_PORT};
...
pub fn apply_hyperparameter_bindings(graph: &mut NodeGraphDefinition, values: &HashMap<String, Value>) {
    for node in &mut graph.nodes {
        ...
        if let Some(tools_value) = node.inline_values.get("tools_config").cloned() {
            if let Ok(mut tools) =
                serde_json::from_value::<Vec<zihuan_graph_engine::brain_tool_spec::BrainToolDefinition>>(tools_value)
            {
