...
const FS_PADDING = 8;
const FS_LINE_HEIGHT = 16;
const FS_SLOT_HEIGHT = 20;
...
function setupFormatStringWidgets(
  ...
  lNode.onDrawForeground = function (this: any, ctx: CanvasRenderingContext2D) {
    ...
    const maxTextWidth = Math.max(20, nodeWidth - FS_PADDING * 2);
    const slotCount = Math.max(this.inputs?.length ?? 0, this.outputs?.length ?? 0);
    const startY = slotCount * FS_SLOT_HEIGHT + FS_PADDING;
    ...
    const boxH = lines.length * FS_LINE_HEIGHT + FS_PADDING;
    const bx = FS_PADDING / 2;
    const bw = nodeWidth - FS_PADDING;
    ...
    let curY = startY + FS_PADDING / 2;
    for (const line of lines) {
      ctx.fillText(line, FS_PADDING, curY);
      curY += FS_LINE_HEIGHT;
    }

    ctx.restore();

    const desired = startY + boxH + FS_PADDING;
