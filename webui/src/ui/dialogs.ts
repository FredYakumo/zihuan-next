...
/** Show a simple error dialog and wait for the user to dismiss it. */
...
// ─── Connection-drop port browser dialogs ────────────────────────────────────

/** Return type for showConnectPortDialog */
export type ConnectPortChoice =
  | { kind: "existing"; targetNodeId: string; targetPortName: string }
  | { kind: "new_node" };

/** Whether two data-type strings are compatible for connection.
 *  Mirrors the patched LiteGraph.isValidConnection logic in canvas.ts. */
function isValidConnectionType(a: string, b: string): boolean {
  if (!a || !b) return true; // wildcard / unknown
  if (a === "*" || b === "*") return true;
...
/**
 * Show the port-browser dialog triggered when releasing a connection on empty canvas.
 * Lists existing graph nodes and their compatible ports, plus a "新建节点" special item.
 * Returns the user's choice or null if cancelled.
 *
 * @param currentNodes   All nodes currently in the graph
 * @param sourceNodeId   Backend id of the node the drag started from
 * @param sourcePortName Name of the port the drag started from
 * @param sourceType     DataType string of the source port
 * @param isFromOutput   true  → dragging from an output port (so we look for INPUT ports to connect to)
 *                       false → dragging from an input port  (so we look for OUTPUT ports to connect to)
 */
export function showConnectPortDialog(
  ...
  return new Promise((resolve) => {
    ...
    // Force clear the base .zh-dialog padding with inline style (highest specificity)
    ...
    // ── Render list ──────────────────────────────────────────────────────────
    const done = (choice: ConnectPortChoice) => { overlay.remove(); resolve(choice); };
    ...
    function renderList(query: string) {
      ...
      for (const node of currentNodes) {
        ...
        // Filter visible + compatible ports
        ...
        for (const p of compatiblePorts) {
          ...
          item.addEventListener("click", () =>
            done({ kind: "existing", targetNodeId: node.id, targetPortName: p.name })
          );
...
/**
 * When a newly created node has multiple ports matching the source data type,
 * show this disambiguation dialog so the user can pick which port to connect to.
 */
  ...
  return new Promise((resolve) => {
    ...
    const renderGroup = (label: string, items: PortSelectOption[]) => {
      ...
      for (const p of items) {
        ...
        row.addEventListener("mouseenter", () => { row.style.background = "var(--btn-hover)"; row.style.borderColor = "var(--link)"; });
        row.addEventListener("mouseleave", () => { row.style.background = "transparent"; row.style.borderColor = "var(--border)"; });
        row.innerHTML = `
        ...
        `;
        row.addEventListener("click", () => { resolve(p); close(); });
...
export function showWorkflowBrowserDialog(workflows: WorkflowEntry[]): Promise<string | null> {
  ...
  return new Promise((resolve) => {
    ...
    // Build a card element for a given workflow entry
    ...
    function renderGrid(query: string): void {
      ...
      const filtered = q
        ? workflows.filter((wf) => {
            ...
            // Fuzzy: every char in q must appear in order inside haystack
            let pos = 0;
            for (const ch of q) {
