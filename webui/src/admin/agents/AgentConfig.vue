...
const availableTools = computed(() => {
  const defaults = [...QQ_CHAT_DEFAULT_TOOLS.map((tool) => ({ id: tool.id, name: tool.label })), ...WORKSPACE_DEFAULT_TOOLS.filter((tool) => !QQ_CHAT_DEFAULT_TOOLS.some((item) => item.id === tool.id)).map((tool) => ({ id: tool.id, name: tool.label }))];
...
const form = reactive<SubAgentDefinition>({ id: "", name: "", description: "", builtin: false, inputs: [], outputs: [], system_prompt: "", user_prompt: "", prompt_parts: [], output_mode: "json_ports", llm_kind: "main", progress_message: "", include_graph_tools: false, run_duration: "Short", tool_ids: [] });
...
function startCreate() { showTypePicker.value = false; isCreating.value = true; error.value = ""; Object.assign(form, { id: "", name: "", description: "", builtin: false, inputs: [], outputs: [], system_prompt: "", user_prompt: "", prompt_parts: [], output_mode: "json_ports", llm_kind: "main", progress_message: "", include_graph_tools: false, run_duration: "Short", tool_ids: [] }); editorVisible.value = true; }
