...
export function useChat(props: ChatProps, emit: ChatEmit) {
    ...
    const canAcceptImageInput = computed(() => {
        ...
        const agentType = selectedService.value?.role_service_type as Record<string, unknown> | undefined;
        const defaultToolsEnabled = agentType?.default_tools_enabled as Record<string, unknown> | undefined;
        if (defaultToolsEnabled?.image_understand === false) {
            return false;
        }
        const configuredModelId = String(agentType?.image_understand_llm_ref_id ?? "");
        return imageUnderstandingModels.value.some((model) => model.config_id === configuredModelId);
    });
    ...
    const canSend = computed(() =>
        ...
        (canAcceptImageInput.value || draftImageAttachments.value.length === 0) &&
    ...
    async function refreshPendingCommandApproval(): Promise<void> {
        ...
        pendingCommandApproval.value = pending;
        if (pending) {
    ...
    function isCurrentCommandConfirmation(liveCall: LiveToolCall): boolean {
        ...
        const pending = pendingCommandApproval.value;
        return !!confirmation && !!pending && confirmation.command === pending.command && confirmation.shell === pending.shell;
    }
    ...
    async function openSession(sessionId: string) {
        ...
        const workspaceChangeResult = await chat.listWorkspaceChanges(sessionId);
        if (revision !== sessionOpenRevision || activeSessionId.value !== sessionId) return;
        workspaceChanges.value = workspaceChangeResult.changes;
        ...
        if (serviceSwitched) {
            preservedServiceChangeId = firstRecord.agent_id;
    ...
    async function submitEditingMessage() {
        ...
        if (!canAcceptImageInput.value && attachments.length > 0) {
    ...
    watch(selectedServiceId, async (serviceId) => {
        const preserveSession = preservedServiceChangeId === serviceId;
        preservedServiceChangeId = null;
        if (!preserveSession) {
            startNewSession();
        }
    ...
    async function decideCommandConfirmation(liveCall: LiveToolCall, decision: "once" | "session" | "reject") {
        ...
        try {
            ...
            if (/no matching pending command approval|no command need to approval/i.test(message)) {
                // The command may have been confirmed by another tab or an older
                // stale dialog. Refresh the server-owned queue and silently retire it.
                confirmation.decision = decision;
                await refreshPendingCommandApproval();
                return;
            }
            showChatError(`命令确认失败: ${message}`);
        }
    }
    ...
    async function sendMessageWithText(rawInput: string, fromAskUser: boolean, options: SendMessageOptions = {}) {
        ...
        if (!fromAskUser && sentAttachments.length > 0 && !canAcceptImageInput.value) {
            showChatError("当前模型不支持多模态输入。请选择支持多模态的模型后再发送图片。");
            return;
        }
