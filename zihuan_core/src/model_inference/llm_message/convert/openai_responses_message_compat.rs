...
use tokio::sync::mpsc;
use crate::llm::{InferenceParam, LLMMessage, StreamToken};
...
pub fn build_responses_message_compat_request_body(
    model_name: &str,
...
pub fn parse_responses_message_compat_response(api_resp: &Value) -> Option<LLMMessage> {
    parse_responses_response(api_resp)
}

pub fn parse_responses_message_compat_sse_response(response_text: &str) -> Option<LLMMessage> {
    parse_responses_sse_response(response_text)
}

pub async fn parse_responses_message_compat_sse_stream_response(
