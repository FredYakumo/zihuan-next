...
use crate::storage_handler::resource_resolver::find_connection;
use crate::storage_handler::rustfs::build_s3_ref as build_s3_direct_ref;
use crate::storage_handler::weaviate::build_weaviate_ref as build_weaviate_direct_ref;
use crate::storage_handler::{
    ...
    async fn build_runtime_instance(&self, config_id: &str) -> Result<(StorageRuntimeInstance, StorageRuntimeHandle)> {
        ...
        let (payload, kind, runtime) = match &connection.kind {
            ...
            ConnectionKind::Rustfs(rustfs) => {
                let s3_ref = crate::runtime::block_async(build_s3_direct_ref(
                    &rustfs.endpoint,
                    &rustfs.bucket,
                    &rustfs.access_key,
                    &rustfs.secret_key,
...
pub fn cleanup_runtime_storage_instances() -> Result<usize> {
    crate::runtime::block_async(RuntimeStorageConnectionManager::shared().cleanup_stale_instances())
}
