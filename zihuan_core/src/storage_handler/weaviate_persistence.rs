...
use crate::storage_handler::WeaviateClient;
...
pub fn upsert_qq_message_list(
    ...
    let media_json = {
        ...
        (!records.is_empty()).then(|| serde_json::to_string(&records)).transpose()?
    };
    let properties = json!({
...
pub fn upsert_image_record(
    ...
    vectors.insert("description_vector".to_string(), description_vector.to_vec());
...
fn required_non_empty_string(value: &str, field_name: &str) -> Result<String> {
    ...
    if trimmed.is_empty() {
        return Err(Error::ValidationError(format!("{field_name} must not be empty")));
    }
