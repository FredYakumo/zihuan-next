...
use crate::graph_engine::function_graph::{FunctionPortDef, FUNCTION_SIGNATURE_PORT};
use crate::graph_engine::graph_boundary::graph_outputs_ports;
use crate::graph_engine::{DataValue, Node, Port};
use crate::error::{Error, Result};

pub struct GraphOutputsNode {
    id: String,
    name: String,
    signature: Vec<FunctionPortDef>,
}

impl GraphOutputsNode {
...
impl Node for GraphOutputsNode {
    fn id(&self) -> &str {
        &self.id
    }
    ...
    fn description(&self) -> Option<&str> {
        Some("主节点图输出边界，将声明的输出端口聚合为结果")
    }

    fn input_ports(&self) -> Vec<Port> {
        let mut ports = vec![crate::graph_engine::function_graph::hidden_function_signature_port()];
        ports.extend(graph_outputs_ports(&self.signature));
        ports
    }
    ...
    fn apply_inline_config(&mut self, inline_values: &crate::graph_engine::NodeConfigFlow) -> Result<()> {
        match inline_values.get(FUNCTION_SIGNATURE_PORT) {
            ...
            Some(other) => Err(Error::ValidationError(format!(
                "graph_outputs.signature 需要 Json，实际为 {}",
                other.data_type()
            ))),
            None => Ok(()),
        }
    ...
    fn execute(&mut self, inputs: crate::graph_engine::NodeInputFlow) -> Result<crate::graph_engine::NodeOutputFlow> {
        ...
        Ok(crate::graph_engine::NodeOutputFlow::new())
    }
}
