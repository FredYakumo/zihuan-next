...
use crate::graph_engine::function_graph::{FunctionPortDef, FUNCTION_RUNTIME_VALUES_PORT, FUNCTION_SIGNATURE_PORT};
use crate::graph_engine::graph_boundary::graph_inputs_ports;
use crate::graph_engine::util::function::data_value_from_json_with_declared_type;
use crate::graph_engine::{DataValue, Node, Port};
...
pub struct GraphInputsNode {
    ...
    runtime_values: Option<crate::graph_engine::RuntimeValueFlow>,
}
...
impl Node for GraphInputsNode {
    ...
    fn input_ports(&self) -> Vec<Port> {
        vec![
            crate::graph_engine::function_graph::hidden_function_signature_port(),
            crate::graph_engine::function_graph::hidden_function_runtime_values_port(),
        ]
    }
    ...
    fn apply_inline_config(&mut self, inline_values: &crate::graph_engine::NodeConfigFlow) -> Result<()> {
    ...
    fn set_function_runtime_values(&mut self, values: crate::graph_engine::RuntimeValueFlow) -> Result<()> {
    ...
    fn execute(&mut self, inputs: crate::graph_engine::NodeInputFlow) -> Result<crate::graph_engine::NodeOutputFlow> {
        ...
        let mut outputs = crate::graph_engine::node_output_flow![];
        for port in &self.signature {
            ...
            let runtime_values = match inputs.get(FUNCTION_RUNTIME_VALUES_PORT) {
                ...
                Some(DataValue::Json(Value::Null)) | None => return Ok(crate::graph_engine::NodeOutputFlow::new()),
...
mod tests {
    ...
    use crate::graph_engine::function_graph::FUNCTION_SIGNATURE_PORT;
    use crate::graph_engine::{DataType, DataValue, Node};

    use super::GraphInputsNode;

    fn test_signature() -> Vec<crate::graph_engine::function_graph::FunctionPortDef> {
        vec![
            crate::graph_engine::function_graph::FunctionPortDef {
    ...
    fn build_node() -> GraphInputsNode {
        ...
        node.apply_inline_config(&crate::graph_engine::NodeConfigFlow::from(HashMap::from([(
    ...
    fn graph_inputs_skip_missing_optional_runtime_value() {
        ...
        node.set_function_runtime_values(crate::graph_engine::RuntimeValueFlow::from(HashMap::from([(
            "required_text".to_string(),
            DataValue::String("hello".to_string()),
        )])))
        .expect("runtime values should apply");

        let outputs = node.execute(crate::graph_engine::NodeInputFlow::new()).expect("execute should succeed");
    ...
    fn graph_inputs_still_require_required_runtime_value() {
        let mut node = build_node();
        node.set_function_runtime_values(crate::graph_engine::RuntimeValueFlow::new())
            .expect("runtime values should apply");

        let error = node.execute(crate::graph_engine::NodeInputFlow::new()).expect_err("execute should fail");
    ...
    fn graph_inputs_json_runtime_values_skip_missing_optional() {
        let mut node = build_node();
        let outputs = node
            .execute(crate::graph_engine::NodeInputFlow::from(HashMap::from([(
                crate::graph_engine::function_graph::FUNCTION_RUNTIME_VALUES_PORT.to_string(),
                DataValue::Json(json!({
                    "required_text": "hello"
                })),
            )])))
