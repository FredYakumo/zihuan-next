use crate::graph_engine::{DataType, DataValue, Node, NodeConfigField, NodeConfigFlow};
...
impl NodeRegistry {
    ...
    pub fn get_node_ports(&self, type_id: &str) -> Option<(Vec<crate::graph_engine::Port>, Vec<crate::graph_engine::Port>)> {
        let factories = self.factories.read().unwrap();
...
macro_rules! register_node {
    ($type_id:expr, $display_name:expr, $category:expr, $description:expr, $node_struct:ty) => {
        $crate::graph_engine::registry::NODE_REGISTRY
            .register(
                $type_id,
                $display_name,
                $category,
...
pub fn build_node_graph_from_definition(definition: &crate::graph_engine::graph_io::NodeGraphDefinition) -> Result<crate::graph_engine::NodeGraph> {
    let mut graph = crate::graph_engine::NodeGraph::new();
...
pub(crate) fn json_to_data_value(json: &Value, target_type: &DataType) -> Option<DataValue> {
    match (json, target_type) {
        ...
        (_, DataType::Image) => serde_json::from_value::<crate::graph_engine::data_value::ImageData>(json.clone())
            .ok()
            .map(DataValue::Image),
