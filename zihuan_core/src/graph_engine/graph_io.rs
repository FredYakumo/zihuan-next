...
use crate::graph_engine::brain_tool_spec::{
...
use crate::graph_engine::data_value::DataType;
...
use crate::graph_engine::graph_boundary::sync_root_graph_io;
use crate::graph_engine::{Node, NodeConfigFlow, NodeGraph, NodeOutputFlow, Port};
...
pub struct NodeGraphDefinition {
    ...
    #[serde(default)]
    pub graph_inputs: Vec<crate::graph_engine::function_graph::FunctionPortDef>,
    #[serde(default)]
    pub graph_outputs: Vec<crate::graph_engine::function_graph::FunctionPortDef>,
    #[serde(default)]
...
fn refresh_port_types_internal(graph: &mut NodeGraphDefinition) {
    use crate::graph_engine::registry::NODE_REGISTRY;
...
pub fn refresh_node_dynamic_ports(node: &mut NodeDefinition) {
    use crate::graph_engine::registry::{json_to_data_value, NODE_REGISTRY};
...
fn fix_function_node_input_types_from_edges(graph: &mut NodeGraphDefinition) {
    use crate::graph_engine::function_graph::{embedded_function_config_from_node, sync_function_node_definition};
        ...
        match NODE_REGISTRY.get_node_ports(&node.node_type) {
            ...
            Some((canonical_inputs, canonical_outputs)) => {
                ...
                for field in &config_fields {
                    if field.required && !node.inline_values.contains_key(&field.key) {
                        issues.push(ValidationIssue::error(format!(
                            "节点 \"{}\" 缺少必要配置项 \"{}\"",
                            node.name, field.key
                        )));
