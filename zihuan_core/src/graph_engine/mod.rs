...
impl NodeGraph {
    ...
    fn port_binding_hp_name(&self, node_id: &str, port_name: &str) -> Option<String> {
        self.definition
            ...
            .port_bindings
            .get(port_name)
            .filter(|b| b.kind == crate::graph_engine::graph_io::PortBindingKind::Hyperparameter)
            .map(|b| b.name.clone())
    }
    ...
    fn reset_runtime_variables_from_definition(&mut self) {
        ...
        for variable in &definition.variables {
            ...
            if let Some(data_value) = crate::graph_engine::registry::json_to_data_value(initial_value, &variable.data_type) {
    ...
    pub fn add_node(&mut self, node: Box<dyn Node>) -> Result<()> {
        ...
        if self.nodes.contains_key(&id) {
            return Err(crate::validation_error!("Node with id '{}' already exists", id));
        }
    ...
    fn runtime_bound_variable_value(&self, node_id: &str, port_name: &str) -> Option<DataValue> {
        ...
        if binding.kind != crate::graph_engine::graph_io::PortBindingKind::Variable {
            return None;
        }
