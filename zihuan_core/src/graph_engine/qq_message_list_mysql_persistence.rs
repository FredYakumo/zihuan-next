use crate::graph_engine::message_mysql_chunking::{
...
use crate::graph_engine::{node_input, node_output, DataType, DataValue, Node, NodeType, Port};
use chrono::Local;
...
impl Node for QQMessageListMySQLPersistenceNode {
    ...
    fn execute(&mut self, inputs: crate::graph_engine::NodeInputFlow) -> Result<crate::graph_engine::NodeOutputFlow> {
        ...
        let (msg_item_type, msg_items) = inputs
            ...
            .ok_or_else(|| crate::error::Error::InvalidNodeInput("qq_message_list is required".to_string()))?;
        ...
        let raw_message_id = inputs
            ...
            .ok_or_else(|| crate::error::Error::InvalidNodeInput("message_id is required".to_string()))?;
        let message_id =
        ...
        let sender_name = inputs
            ...
            .ok_or_else(|| crate::error::Error::InvalidNodeInput("sender_name is required".to_string()))?;
        ...
        let mysql_config = match rdb_pool {
            crate::data_refs::RelationalDbConnection::MySql(config) => config,
            _ => return Err(crate::error::Error::InvalidNodeInput("mysql_ref must be a MySQL connection".to_string())),
        };
        ...
        let pool = match mysql_config.pool.clone() {
            ...
            None => {
                ...
                return crate::graph_engine::return_with_node_output![self;
