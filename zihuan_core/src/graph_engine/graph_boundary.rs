...
use crate::graph_engine::graph_io::{GraphPosition, GraphSize, NodeDefinition, NodeGraphDefinition};
use crate::graph_engine::Port;
...
fn agent_event_ports() -> Vec<crate::graph_engine::function_graph::FunctionPortDef> {
    use crate::graph_engine::function_graph::FunctionPortDef;
    use crate::graph_engine::DataType;
    vec![
        FunctionPortDef {
...
pub fn root_graph_to_tool_subgraph(graph: &NodeGraphDefinition) -> NodeGraphDefinition {
    ...
    let input_signature = subgraph
        ...
        .iter()
        .find(|node| node.id == GRAPH_INPUTS_NODE_ID)
        .and_then(|node| crate::graph_engine::function_graph::function_signature_from_inline_values(&node.inline_values))
    ...
    for node in &mut subgraph.nodes {
        if node.id == GRAPH_INPUTS_NODE_ID {
            node.id = crate::graph_engine::function_graph::FUNCTION_INPUTS_NODE_ID.to_string();
            node.name = "函数输入".to_string();
            node.node_type = crate::graph_engine::function_graph::FUNCTION_INPUTS_NODE_TYPE.to_string();
        } else if node.id == GRAPH_OUTPUTS_NODE_ID {
            node.id = crate::graph_engine::function_graph::FUNCTION_OUTPUTS_NODE_ID.to_string();
            node.name = "函数输出".to_string();
            node.node_type = crate::graph_engine::function_graph::FUNCTION_OUTPUTS_NODE_TYPE.to_string();
        }
    }

    for edge in &mut subgraph.edges {
        if edge.from_node_id == GRAPH_INPUTS_NODE_ID {
            edge.from_node_id = crate::graph_engine::function_graph::FUNCTION_INPUTS_NODE_ID.to_string();
        }
        if edge.to_node_id == GRAPH_OUTPUTS_NODE_ID {
            edge.to_node_id = crate::graph_engine::function_graph::FUNCTION_OUTPUTS_NODE_ID.to_string();
        }
    }

    subgraph.graph_inputs.clear();
    subgraph.graph_outputs.clear();
    crate::graph_engine::function_graph::sync_function_subgraph_signature(&mut subgraph, &input_signature, &output_signature);
    subgraph
}
