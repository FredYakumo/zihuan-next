use crate::ims_bot_adapter::send_qq_message_batches::{describe_message_segments, qq_messages_from_data_value};
use crate::ims_bot_adapter::ws_action::{json_i64, qq_message_list_to_send_json, response_message_id, response_success, ws_send_action};
use log::{info, warn};
...
impl Node for SendMessageNode {
    ...
    fn execute(&mut self, inputs: crate::graph_engine::NodeInputFlow) -> Result<crate::graph_engine::NodeOutputFlow> {
        ...
        let adapter_ref = match inputs.get("ims_bot_adapter") {
            Some(DataValue::BotAdapterRef(handle)) => crate::ims_bot_adapter::adapter::shared_from_handle(handle),
