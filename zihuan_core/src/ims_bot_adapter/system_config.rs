...
use crate::ims_bot_adapter::adapter::{BotAdapter, BotAdapterConfig, SharedBotAdapter};
use crate::storage_handler::{save_connections, ConnectionConfig, ConnectionKind};
...
pub fn load_ims_bot_adapter_connections() -> Result<Vec<BotAdapterConnectionConfig>> {
    Ok(crate::storage_handler::load_connections()?
        .into_iter()
...
pub fn save_ims_bot_adapter_connections(connections: Vec<BotAdapterConnectionConfig>) -> Result<()> {
    let mut all = crate::storage_handler::load_connections()?;
