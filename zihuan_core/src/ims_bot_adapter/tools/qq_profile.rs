...
use crate::ims_bot_adapter::login_info::{parse_login_info, qq_avatar_url};
use crate::ims_bot_adapter::message_helpers::get_bot_id;
use crate::ims_bot_adapter::models::MessageEvent;
use crate::ims_bot_adapter::ws_action::ws_send_action;
...
fn fallback_get_login_info(
    adapter: &SharedBotAdapter,
    s3_ref: &Option<Arc<S3Ref>>,
) -> Result<Option<crate::ims_bot_adapter::login_info::BotLoginInfo>> {
...
fn resolve_avatar_media(user_id: &str, s3_ref: &Option<Arc<S3Ref>>) -> Option<PersistedMedia> {
    ...
    let cached = match crate::runtime::block_async(async { s3.get_object_bytes(&key).await }) {
...
fn is_group_event(event: &MessageEvent) -> bool {
    matches!(event.message_type, crate::ims_bot_adapter::models::event_model::MessageType::Group)
}
