...
use crate::storage_handler::{find_connection, load_connections, ConnectionConfig, ConnectionKind};
...
use crate::ims_bot_adapter::adapter::{BotAdapter, SharedBotAdapter};
use crate::ims_bot_adapter::ws_action::ws_send_action_async;
use crate::ims_bot_adapter::{build_ims_bot_adapter, parse_ims_bot_adapter_connection};
...
impl ActiveAdapterManager {
    ...
    pub async fn get_active_bot_adapter_handle(
        &self,
        config_id: &str,
    ) -> Result<crate::ims_bot_adapter::BotAdapterHandle> {
        let adapter = self.get_or_create(config_id).await?;
        let handle: crate::ims_bot_adapter::BotAdapterHandle = adapter;
        Ok(handle)
    }
...
pub fn get_active_bot_adapter_handle(connection_id: &str) -> Option<crate::ims_bot_adapter::BotAdapterHandle> {
    crate::runtime::block_async(ActiveAdapterManager::shared().get_active_bot_adapter_handle(connection_id)).ok()
}

pub fn has_active_bot_adapter(connection_id: &str) -> bool {
    crate::runtime::block_async(async move {
...
pub fn stop_active_bot_adapter(connection_id: &str) -> bool {
    crate::runtime::block_async(ActiveAdapterManager::shared().close_instances_for_config(connection_id))
        .map(|count| {
            if count > 0 {
                info!(
