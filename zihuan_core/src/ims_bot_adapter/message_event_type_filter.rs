use crate::ims_bot_adapter::models::event_model::MessageType;
...
impl Node for MessageEventTypeFilterNode {
    ...
    node_input![
        port! { name = "message_event", ty = crate::ims_bot_adapter::models::event_model::MessageEvent, desc = "输入的消息事件" },
        port! { name = "filter_type",   ty = String,       desc = "过滤类型：private（好友消息）或 group（群组消息）", optional },
    ];

    node_output![
        port! { name = "true_event",  ty = crate::ims_bot_adapter::models::event_model::MessageEvent, desc = "消息类型匹配时的输出" },
        port! { name = "false_event", ty = crate::ims_bot_adapter::models::event_model::MessageEvent, desc = "消息类型不匹配时的输出" },
    ];
