...
use crate::ims_bot_adapter::ws_action::ws_send_action_async;
use crate::storage_handler::{enrich_event_images, enrich_message_images, ImageCacheAdapter, PendingImageUpload};
use tokio::sync::Mutex as TokioMutex;
use tokio::sync::{mpsc, oneshot};
use crate::error::Result;
use crate::ims_bot_adapter::models::message::{ForwardNodeMessage, Message};
use crate::url_utils::extract_host;
use crate::graph_engine::message_restore::restore_message_snapshot;
use crate::graph_engine::object_storage::S3Ref;
...
impl ImageCacheAdapter for BotAdapterImageCacheHandle {
    ...
    async fn enqueue_pending_image(&self, pending: PendingImageUpload) -> bool {
        ...
        let (queue, running_flag) = {
            ...
            (guard.pending_image_uploads.clone(), guard.image_retry_task_running.clone())
        };
...
pub fn shared_from_handle(handle: &crate::ims_bot_adapter::BotAdapterHandle) -> SharedBotAdapter {
    handle
        .clone()
        ...
        match segment_type {
            "image" => {
                ...
                messages.push(Message::Image(
                    crate::ims_bot_adapter::models::message::ImageMessage::new(
                        crate::ims_bot_adapter::models::message::PersistedMedia::new(
                            crate::ims_bot_adapter::models::message::PersistedMediaSource::QqChat,
                            original_source,
                            String::new(),
            ...
            "forward" => {
                messages.push(Message::Forward(
                    crate::ims_bot_adapter::models::message::ForwardMessage {
...
fn parse_forward_node_value(value: &serde_json::Value) -> Option<ForwardNodeMessage> {
    ...
    let nickname = node
        ...
        .or_else(|| node.get("name").and_then(|value| value.as_str()).map(ToString::to_string))
        .or_else(|| {
            sender
    ...
    Some(ForwardNodeMessage { user_id, nickname, id, content })
}
...
async fn fetch_forward_content(adapter: &SharedBotAdapter, forward_id: &str) -> Result<Vec<ForwardNodeMessage>> {
    ...
    let raw_messages = payload
        ...
        .unwrap_or_default();
    let nodes = raw_messages.iter().filter_map(parse_forward_node_value).collect::<Vec<_>>();
