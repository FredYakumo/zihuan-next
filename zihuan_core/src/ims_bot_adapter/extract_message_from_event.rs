use crate::ims_bot_adapter::adapter::{restore_message_list_for_message_id, shared_from_handle};
use crate::ims_bot_adapter::models::message::MessageProp;
use log::{info, warn};
...
use crate::graph_engine::{node_input, node_output, DataType, DataValue, Node, NodeInputFlow, Port};

use crate::ims_bot_adapter::models::message::Message;
use crate::ims_bot_adapter::multimodal_image_url::{resolve_image_message_part, resolve_plain_text_segments, ResolvedTextSegment};
...
impl ExtractMessageFromEventNode {
            ...
            match message {
                ...
                Message::Reply(reply) => {
                    ...
                    if include_reply_source_block {
                        if let Some(source_messages) = reply.message_source.as_deref() {
                            ...
                            text_buffer.push_str(&format!("[{}]\n", crate::ims_bot_adapter::REPLAY_CONTENT_LABEL));
                ...
                Message::Forward(forward) => {
                    if forward.content.is_empty() {
                        ...
                        text_buffer.push_str(&format!("[{}]\n", crate::ims_bot_adapter::FORWARD_CONTENT_LABEL));
    ...
    fn build_user_message(messages: &[Message], msg_prop: &MessageProp, s3_ref: Option<&S3Ref>) -> LLMMessage {
        ...
        if let Some(ref_cnt) = msg_prop.ref_content.as_deref().filter(|value| !value.is_empty()) {
            if text_buffer.contains(crate::ims_bot_adapter::REPLAY_CONTENT_LABEL) {
                if !text_buffer.is_empty() {
                    text_buffer.push_str("\n\n");
                }
                text_buffer.push_str(&format!("[{}]\n", crate::ims_bot_adapter::QUOTE_CONTENT_APPENDIX_LABEL));
...
mod tests {
    ...
    use crate::ims_bot_adapter::models::message::{ImageMessage, Message, PersistedMedia, PersistedMediaSource, PlainTextMessage};
}
...
impl Node for ExtractMessageFromEventNode {
    ...
    fn execute(&mut self, inputs: crate::graph_engine::NodeInputFlow) -> Result<crate::graph_engine::NodeOutputFlow> {
        ...
        outputs.insert(
            "messages".to_string(),
            DataValue::Vec(
                Box::new(crate::graph_engine::DataType::LLMMessage),
