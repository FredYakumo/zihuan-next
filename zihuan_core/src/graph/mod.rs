...
impl NodeGraph {
    ...
    fn port_binding_hp_name(&self, node_id: &str, port_name: &str) -> Option<String> {
        self.definition
            ...
            .port_bindings
            .get(port_name)
            .filter(|b| b.kind == crate::graph::graph_io::PortBindingKind::Hyperparameter)
            .map(|b| b.name.clone())
    }
    ...
    fn execute_and_collect_node_results(
        &mut self,
    ...
    fn runtime_bound_variable_value(&self, node_id: &str, port_name: &str) -> Option<DataValue> {
        ...
        if binding.kind != crate::graph::graph_io::PortBindingKind::Variable {
            return None;
        }
