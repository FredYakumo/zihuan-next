use serde_json::Value;

use crate::error::{Error, Result};
use crate::graph::function_graph::{FunctionPortDef, FUNCTION_SIGNATURE_PORT};
use crate::graph::graph_boundary::graph_outputs_ports;
use crate::graph::{DataValue, Node, Port};

pub struct GraphOutputsNode {
    id: String,
    name: String,
    signature: Vec<FunctionPortDef>,
}

impl GraphOutputsNode {
    pub fn new(id: impl Into<String>, name: impl Into<String>) -> Self {
        Self {
            id: id.into(),
            name: name.into(),
            signature: Vec::new(),
        }
    }

    fn apply_signature_json(&mut self, value: &Value) -> Result<()> {
        self.signature =
            serde_json::from_value::<Vec<FunctionPortDef>>(value.clone()).map_err(|_| {
                Error::ValidationError(
                    "graph_outputs.signature 不是有效的节点图签名 JSON".to_string(),
                )
            })?;
        Ok(())
    }
}

impl Node for GraphOutputsNode {
    fn id(&self) -> &str {
        &self.id
    }

    fn name(&self) -> &str {
        &self.name
    }

    fn description(&self) -> Option<&str> {
        Some("主节点图输出边界，将声明的输出端口聚合为结果")
    }

    fn input_ports(&self) -> Vec<Port> {
        let mut ports = vec![crate::graph::function_graph::hidden_function_signature_port()];
        ports.extend(graph_outputs_ports(&self.signature));
        ports
    }

    fn output_ports(&self) -> Vec<Port> {
        Vec::new()
    }

    fn has_dynamic_input_ports(&self) -> bool {
        true
    }

    fn apply_inline_config(&mut self, inline_values: &crate::graph::NodeConfigFlow) -> Result<()> {
        match inline_values.get(FUNCTION_SIGNATURE_PORT) {
            Some(DataValue::Json(value)) => self.apply_signature_json(value),
            Some(other) => Err(Error::ValidationError(format!(
                "graph_outputs.signature 需要 Json，实际为 {}",
                other.data_type()
            ))),
            None => Ok(()),
        }
    }

    fn execute(
        &mut self,
        inputs: crate::graph::NodeInputFlow,
    ) -> Result<crate::graph::NodeOutputFlow> {
        if let Some(DataValue::Json(value)) = inputs.get(FUNCTION_SIGNATURE_PORT) {
            self.apply_signature_json(value)?;
        }
        self.validate_inputs(&inputs)?;
        Ok(crate::graph::NodeOutputFlow::new())
    }
}
