...
use crate::graph::function_graph::{FunctionPortDef, FUNCTION_RUNTIME_VALUES_PORT, FUNCTION_SIGNATURE_PORT};
use crate::graph::graph_boundary::graph_inputs_ports;
...
pub struct GraphInputsNode {
    ...
    runtime_values: Option<crate::graph::RuntimeValueFlow>,
}
...
impl Node for GraphInputsNode {
    ...
    fn input_ports(&self) -> Vec<Port> {
        vec![
            crate::graph::function_graph::hidden_function_signature_port(),
            crate::graph::function_graph::hidden_function_runtime_values_port(),
        ]
    }
    ...
    fn execute(&mut self, inputs: crate::graph::NodeInputFlow) -> Result<crate::graph::NodeOutputFlow> {
        ...
        let mut outputs = crate::graph::node_output_flow![];
        for port in &self.signature {
            ...
            let runtime_values = match inputs.get(FUNCTION_RUNTIME_VALUES_PORT) {
                ...
                Some(DataValue::Json(Value::Null)) | None => return Ok(crate::graph::NodeOutputFlow::new()),
...
mod tests {
    ...
    use crate::graph::function_graph::FUNCTION_SIGNATURE_PORT;
    use crate::graph::{DataType, DataValue, Node};

    use super::GraphInputsNode;

    fn test_signature() -> Vec<crate::graph::function_graph::FunctionPortDef> {
        vec![
            crate::graph::function_graph::FunctionPortDef {
    ...
    fn build_node() -> GraphInputsNode {
        ...
        node.apply_inline_config(&crate::graph::NodeConfigFlow::from(HashMap::from([(
    ...
    fn graph_inputs_skip_missing_optional_runtime_value() {
        ...
        node.set_function_runtime_values(crate::graph::RuntimeValueFlow::from(HashMap::from([(
    ...
    fn graph_inputs_json_runtime_values_skip_missing_optional() {
        let mut node = build_node();
        let outputs = node
            .execute(crate::graph::NodeInputFlow::from(HashMap::from([(
                crate::graph::function_graph::FUNCTION_RUNTIME_VALUES_PORT.to_string(),
                DataValue::Json(json!({
                    "required_text": "hello"
                })),
            )])))
