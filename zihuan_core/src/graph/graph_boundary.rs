...
fn agent_event_ports() -> Vec<crate::graph::function_graph::FunctionPortDef> {
...
pub fn root_graph_to_tool_subgraph(graph: &NodeGraphDefinition) -> NodeGraphDefinition {
    ...
    for node in &mut subgraph.nodes {
        if node.id == GRAPH_INPUTS_NODE_ID {
            node.id = crate::graph::function_graph::FUNCTION_INPUTS_NODE_ID.to_string();
            node.name = "函数输入".to_string();
            node.node_type = crate::graph::function_graph::FUNCTION_INPUTS_NODE_TYPE.to_string();
        } else if node.id == GRAPH_OUTPUTS_NODE_ID {
            node.id = crate::graph::function_graph::FUNCTION_OUTPUTS_NODE_ID.to_string();
            node.name = "函数输出".to_string();
            node.node_type = crate::graph::function_graph::FUNCTION_OUTPUTS_NODE_TYPE.to_string();
        }
    }

    for edge in &mut subgraph.edges {
        if edge.from_node_id == GRAPH_INPUTS_NODE_ID {
            edge.from_node_id = crate::graph::function_graph::FUNCTION_INPUTS_NODE_ID.to_string();
        }
        if edge.to_node_id == GRAPH_OUTPUTS_NODE_ID {
            edge.to_node_id = crate::graph::function_graph::FUNCTION_OUTPUTS_NODE_ID.to_string();
        }
    }
