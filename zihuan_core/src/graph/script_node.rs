...
use crate::graph::data_value::{
    LLMMessageSessionCacheRef, SessionClaim, SessionStateRef, SESSION_CLAIM_CONTEXT,
};
use dynamic_script_engine::{NodeRuntimeConfig, ScriptLanguage};

use super::registry::{NodeFactory, NodeRegistry};
use super::{
    DataValue, Node, NodeConfigField, NodeConfigFlow, NodeInputFlow, NodeOutputFlow, Port,
...
pub fn start_dynamic_script_runtime() -> Result<()> {
    let workspace_root =
        std::env::current_dir().map_err(|error| Error::ValidationError(error.to_string()))?;
...
fn request_dynamic_script_runtime(
    ...
    let workspace_root =
        std::env::current_dir().map_err(|error| Error::ValidationError(error.to_string()))?;
    let config = crate::config::ConfigCenter::shared().load_root()?;
    dynamic_script_engine::request_script_runtime(
        &workspace_root,
        language,
        &config.node_runtime,
        &config.python_runtime,
        request,
        &mut |method, params| host(method, params).map_err(|error| error.to_string()),
        None,
    )
    .map_err(|error| Error::ValidationError(error.to_string()))
}
...
fn script_value_to_json(value: &DataValue) -> Value {
    ...
    match value {
        DataValue::Vec(_, values) => {
            Value::Array(values.iter().map(script_value_to_json).collect())
        }
        _ => value.to_json(),
    }
}
...
fn session_resource(params: &Value) -> Result<Arc<SessionStateRef>> {
    ...
    match resource_from_json(value, &super::DataType::SessionStateRef) {
        ...
        _ => Err(Error::ValidationError(
            "session_ref must be a SessionStateRef handle".to_string(),
        )),
    }
}
...
fn cache_resource(params: &Value) -> Result<Arc<LLMMessageSessionCacheRef>> {
    ...
    match resource_from_json(value, &super::DataType::LLMMessageSessionCacheRef) {
        ...
        _ => Err(Error::ValidationError(
            "cache_ref must be an LLMMessageSessionCacheRef handle".to_string(),
        )),
    }
}

fn llm_model_resource(
    params: &Value,
    field: &str,
) -> Result<Arc<dyn crate::model_inference::llm::llm_base::LLMBase>> {
    let value = params
        .get(field)
...
fn embedding_model_resource(
    ...
    match resource_from_json(value, &super::DataType::EmbeddingModel) {
        ...
        _ => Err(Error::ValidationError(
            "embedding_model must be an EmbeddingModel handle".to_string(),
        )),
    }
}
...
fn qq_messages_param(
    params: &Value,
    field: &str,
) -> Result<Vec<crate::ims_bot_adapter::models::message::Message>> {
...
fn web_search_resource(params: &Value) -> Result<Arc<dyn crate::rag::WebSearchEngine>> {
    ...
    match resource_from_json(value, &super::DataType::WebSearchEngineRef) {
        ...
        _ => Err(Error::ValidationError(
            "tavily_ref must be a WebSearchEngineRef handle".to_string(),
        )),
    }
}
...
fn bot_adapter_resource(
    ...
    match resource_from_json(value, &super::DataType::BotAdapterRef) {
        ...
        _ => Err(Error::ValidationError(format!("{field} must be a BotAdapterRef handle"))),
    }
}

fn optional_s3_resource(
    params: &Value,
    field: &str,
) -> Result<Option<Arc<crate::graph::object_storage::S3Ref>>> {
...
fn mysql_resource(params: &Value, field: &str) -> Result<Arc<crate::data_refs::MySqlConfig>> {
    ...
    match resource_from_json(value, &super::DataType::RdbRef) {
        Some(DataValue::RdbRef(crate::data_refs::RelationalDbConnection::MySql(reference))) => {
            Ok(reference)
        }
        Some(DataValue::RdbRef(_)) => {
            Err(Error::ValidationError(format!("{field} must be a MySQL RdbRef handle")))
        }
...
fn message_event(
    params: &Value,
) -> Result<crate::ims_bot_adapter::runtime::models::event_model::MessageEvent> {
    let value = params
        .get("message_event")
...
fn history_messages_json(rows: Vec<sqlx::mysql::MySqlRow>, limit: usize) -> Value {
    let messages = crate::graph::message_rdb_history_common::format_history_messages(
        crate::graph::message_rdb_history_common::aggregate_history_rows(
            rows.into_iter()
                .map(crate::graph::message_rdb_history_common::message_history_chunk_row_from_row)
                .collect(),
            limit,
        ),
    );
...
fn llm_messages_from_json(
    value: &Value,
    field: &str,
) -> Result<Vec<crate::model_inference::llm::LLMMessage>> {
    let parsed =
        script_json_to_value(value, &super::DataType::Vec(Box::new(super::DataType::LLMMessage)))
            .ok_or_else(|| Error::ValidationError(format!("{field} must be Vec<LLMMessage>")))?;
    match parsed {
        DataValue::Vec(_, items) => items
            .into_iter()
            .map(|item| match item {
...
) -> Result<Vec<DynamicScriptNodeDefinition>> {
...
fn load_ui_template(
    ...
    let template_path = ui.and_then(|value| value.get("template_path")).and_then(Value::as_str);
    let style_path = ui.and_then(|value| value.get("style_path")).and_then(Value::as_str);
    if template_path.is_none() && style_path.is_none() {
        return (None, None);
    }

    let mut document = String::new();
    if let Some(path_value) = style_path {
        match read_ui_file(script_dir, path_value, &["css", "scss"]) {
            Ok((extension, content)) => match ui_file_block(&extension, &content) {
                Ok(block) => document.push_str(&block),
                Err(error) => return (None, Some(error)),
            },
            Err(error) => return (None, Some(error)),
        }
    }
    if let Some(path_value) = template_path {
        match read_ui_file(script_dir, path_value, &["html", "css", "scss"]) {
            Ok((extension, content)) => match ui_file_block(&extension, &content) {
                Ok(block) => document.push_str(&block),
                Err(error) => return (None, Some(error)),
            },
    ...
    (Some(sanitize_ui_template(&document)), None)
}
...
fn read_ui_file(
    ...
    {
        return Err("UI 模板路径必须是相对路径且不能包含 ..".to_string());
    }
    ...
    let canonical_script_dir =
        script_dir.canonicalize().map_err(|error| format!("脚本目录不可用: {error}"))?;
    let canonical = path.canonicalize().map_err(|error| format!("UI 模板读取失败: {error}"))?;

    // Ensure the resolved path stays inside the script directory.
    if !canonical.starts_with(&canonical_script_dir) {
        return Err("UI 模板路径超出脚本目录范围".to_string());
    }
    ...
    if std::fs::metadata(&canonical)
        ...
        .unwrap_or(true)
    {
        return Err("UI 模板超过 512 KiB 限制".to_string());
    }

    let content =
        std::fs::read_to_string(canonical).map_err(|error| format!("UI 模板读取失败: {error}"))?;
    Ok((extension, content))
}

// Wrap raw CSS (or compiled SCSS) so the frontend template injection applies it.
fn style_block(css: &str) -> String {
    format!("<style>\n{css}\n</style>")
}

fn compile_scss(source: &str) -> std::result::Result<String, String> {
    grass::from_string(source.to_owned(), &grass::Options::default())
        .map_err(|error| format!("SCSS 编译失败: {error}"))
}

fn ui_file_block(extension: &str, content: &str) -> std::result::Result<String, String> {
    match extension {
        "html" => Ok(content.to_string()),
        "css" => Ok(style_block(content)),
        "scss" => compile_scss(content).map(|css| style_block(&css)),
        _ => Ok(String::new()),
    }
}
...
fn resolve_script_ports(
    ...
    .map_err(|error| {
        Error::ValidationError(format!("解析动态脚本节点 '{type_id}' 的动态端口失败: {error}"))
    })?;
    serde_json::from_value(response).map_err(|error| {
        Error::ValidationError(format!("动态脚本节点 '{}' 的端口响应无效: {error}", type_id))
    })
}
...
impl Node for DynamicScriptNode {
    ...
    fn execute(&mut self, inputs: NodeInputFlow) -> Result<NodeOutputFlow> {
        ...
        let context = ScriptHostContext {
            ...
            variables: self.runtime_variables.clone().map(ScriptVariableStore::Graph),
        };
        let response = request_dynamic_script_runtime(
            self.definition.language,
            &request,
            &mut |method, params| dispatch_script_host_call(method, params, &context),
        )?;
        ...
        let outputs = response.get("outputs").and_then(Value::as_object).ok_or_else(|| {
            Error::ValidationError(format!(
                "动态脚本节点 '{}' 输出缺少 outputs 对象",
                self.definition.type_id
            ))
        })?;
        let declared = self
            .output_ports
            .iter()
            .map(|port| (port.name.as_str(), &port.data_type))
            .collect::<HashMap<_, _>>();
        let mut result = NodeOutputFlow::new();
        for (name, value) in outputs {
            let Some(data_type) = declared.get(name.as_str()) else {
                return Err(Error::ValidationError(format!(
                    "动态脚本节点 '{}' 返回了未声明输出 '{name}'",
                    self.definition.type_id
                )));
            };
            let parsed = script_json_to_value(value, data_type).ok_or_else(|| {
                Error::ValidationError(format!(
                    "动态脚本节点 '{}' 输出 '{name}' 类型不匹配",
                    self.definition.type_id
                ))
            })?;
...
/// The runtime-variable storage a script's `variables.get` / `variables.set` calls address.
///
/// A dynamic script node writes into its graph's runtime variables; a script tool has no graph,
/// so it writes into the owning node's shared runtime values. Both call the same SDK methods.
#[derive(Clone)]
pub enum ScriptVariableStore {
    Graph(RuntimeVariableStore),
    Shared(Arc<Mutex<HashMap<String, DataValue>>>),
}

impl ScriptVariableStore {
    fn get(&self, name: &str) -> Option<Value> {
        match self {
            Self::Graph(store) => {
                store.read().ok().and_then(|values| values.get(name).map(script_value_to_json))
            }
            Self::Shared(store) => {
                store.lock().ok().and_then(|values| values.get(name).map(script_value_to_json))
            }
        }
    }
    ...
    fn set(&self, name: &str, value: Value) -> Result<()> {
        ...
        match self {
            Self::Graph(store) => {
                store
                    .write()
                    .map_err(|_| Error::ValidationError("运行时变量存储不可用".to_string()))?
                    .insert(name, parsed);
            }
            Self::Shared(store) => {
                store
...
/// Where one script's host calls read and write their execution-scoped state.
#[derive(Clone)]
pub struct ScriptHostContext {
    /// Identifies the caller to the UI event channel, which is keyed by node and task.
    pub node_id: String,
    /// Runtime variable storage, absent when the caller has none (a tool with no owner graph).
    pub variables: Option<ScriptVariableStore>,
}

/// Serves one `zihuan.<namespace>.<method>()` call coming from a running script.
///
/// Scripts reach the host through this single dispatch: dynamic script nodes and agent script
/// tools share it, so both see the same SDK surface, and `ScriptHostContext` supplies the only
/// two things that differ between them — the caller's id and its variable storage.
pub fn dispatch_script_host_call(
    method: &str,
    params: &Value,
    context: &ScriptHostContext,
) -> Result<Value> {
    let node_id = context.node_id.as_str();
    ...
    match method {
        "ui.publish" | "ui.update" => {
            ...
            UI_PUBLISHER.with(|publisher| {
                if let Some(publisher) = publisher.borrow().as_ref() {
                    publisher(&task_id, node_id, state);
                    Ok(Value::Bool(true))
                } else {
                    Err(Error::ValidationError("当前执行上下文未配置 UI 发布器".to_string()))
                }
            })
        }
        "ui.wait_event" => {
            ...
            Ok(wait_dynamic_script_ui_event(&task_id, node_id, event, timeout_ms)
                .unwrap_or(Value::Null))
        }
        "variables.get" => {
            let name = params
            ...
            Ok(variables.as_ref().and_then(|store| store.get(name)).unwrap_or(Value::Null))
        }
        "variables.set" => {
            let name = params
                .get("name")
            ...
            let store = variables
                .as_ref()
                .ok_or_else(|| Error::ValidationError("当前图没有运行时变量存储".to_string()))?;
        ...
        "agent.llm" => {
            let resources = crate::agent::runtime_context::current_agent_resources()?;
            let kind =
                crate::agent::normalize_llm_kind(params.get("llm_kind").and_then(Value::as_str))?;
        ...
        "agent.web_search" => {
            ...
            .ok_or_else(|| {
                Error::ValidationError("web_search_engine_connection_id is required".to_string())
            })?;
        ...
        "bot.sender_from_event" => {
            ...
            let sender =
                crate::ims_bot_adapter::runtime::models::sender_model::Sender::from_message_event(
                    &event,
                )
                .ok_or_else(|| {
                    Error::ValidationError("group message is missing group_id".to_string())
                })?;
        ...
        "bot.send" => {
            ...
            let sender =
                serde_json::from_value::<crate::ims_bot_adapter::models::sender_model::Sender>(
                    params
                        .get("sender")
                        .cloned()
            ...
            let messages =
                serde_json::from_value::<Vec<crate::ims_bot_adapter::models::message::Message>>(
                    params
                        .get("message")
                        .cloned()
                        .ok_or_else(|| Error::ValidationError("message is required".to_string()))?,
                )
                .map_err(|error| {
                    Error::ValidationError(format!("message must be Vec<QQMessage>: {error}"))
                })?;
            ...
            let response = crate::ims_bot_adapter::runtime::ws_action::ws_send_action(
                &adapter,
                action_name,
                params,
            )?;
            ...
            let message_id =
                crate::ims_bot_adapter::runtime::ws_action::response_message_id(&response)
                    .unwrap_or(-1);
        ...
        "bot.send_batches" => {
            ...
            let batches = serde_json::from_value::<
                Vec<Vec<crate::ims_bot_adapter::models::message::Message>>,
            >(params.get("message_batches").cloned().ok_or_else(
                || Error::ValidationError("message_batches is required".to_string()),
            )?)
            .map_err(|error| {
                Error::ValidationError(format!(
                    "message_batches must be Vec<Vec<QQMessage>>: {error}"
                ))
            })?;
            let delay_millis = match params.get("delay_millis") {
                ...
                Some(value) => value.as_i64().filter(|value| *value >= 0).ok_or_else(|| {
                    Error::ValidationError(
                        "delay_millis must be a non-negative integer".to_string(),
                    )
                })? as u64,
            };
        ...
        "bot.extract_messages" => {
            ...
            let target_message_id =
                params.get("message_id").and_then(Value::as_i64).filter(|value| *value > 0);
        ...
        "bot.group_id_from_event" => {
                ...
                != crate::ims_bot_adapter::runtime::models::event_model::MessageType::Group
            {
                return Err(Error::ValidationError(
                    "message_event must be a group message".to_string(),
                ));
            }
            let group_id = event.group_id.ok_or_else(|| {
                Error::ValidationError("group_id is missing in group message event".to_string())
            })?;
        ...
        "bot.optional_group_id_from_event" => {
                ...
                == crate::ims_bot_adapter::runtime::models::event_model::MessageType::Group
            {
                event
                    ...
                    .ok_or_else(|| {
                        Error::ValidationError(
                            "group_id is missing in group message event".to_string(),
                        )
                    })?
                    .to_string()
            } else {
        ...
        "bot.filter_event_type" => {
            let event = message_event(params)?;
            let filter_type =
                optional_string(params, "filter_type").unwrap_or_else(|| "private".to_string());
            let matches = match filter_type.as_str() {
                ...
                _ => event.message_type
                    == crate::ims_bot_adapter::runtime::models::event_model::MessageType::Private,
            };
            let event = serde_json::to_value(event)?;
            Ok(if matches {
                json!({"true_event": event})
            } else {
                json!({"false_event": event})
            })
        }
        "session.get" => {
        ...
        "session.try_claim" => {
            ...
            let (state, claimed) = if let Ok(handle) = tokio::runtime::Handle::try_current() {
                block_in_place(|| {
                    handle.block_on(state_ref.try_claim(&sender_for_claim, desired_state))
                })
            } else {
                tokio::runtime::Runtime::new()?
                    .block_on(state_ref.try_claim(&sender_for_claim, desired_state))
            };
            if claimed {
                if let (Ok(context), Some(claim_token)) =
                    (SESSION_CLAIM_CONTEXT.try_with(Arc::clone), state.claim_token())
                {
                    context.register_claim(SessionClaim { session_ref, sender_id, claim_token });
                }
            }
            Ok(
                json!({"claimed": claimed, "in_session": state.in_session, "state_json": state.state_json}),
            )
        }
        "session.release" => {
            let session_ref = session_resource(params)?;
            ...
            let released = if let Ok(handle) = tokio::runtime::Handle::try_current() {
                ...
                tokio::runtime::Runtime::new()?
                    .block_on(session_ref.release(&sender_id, claim_token))
            };
        ...
        "message_cache.append" => {
            ...
            let messages = llm_messages_from_json(
                params
                    .get("messages")
                    .ok_or_else(|| Error::ValidationError("messages is required".to_string()))?,
                "messages",
            )?;
        ...
        "model.compact_context" => {
            ...
            let compact_context_length = params
                .get("compact_context_length")
                .and_then(Value::as_i64)
                .filter(|value| *value > 0)
                .unwrap_or_default() as usize;
            let force_compact =
                params.get("force_compact").and_then(Value::as_bool).unwrap_or(false);
        ...
        "model.create_llm_from_ref" => {
            ...
            let llm_ref = crate::config::llm_refs::load_llm_refs()?
                ...
                .ok_or_else(|| {
                    Error::ValidationError(format!("llm_ref '{}' not found", llm_ref_id))
                })?;
            if !llm_ref.enabled {
                return Err(Error::ValidationError(format!(
                    "llm_ref '{}' is disabled",
            ...
            let texts = params
                ...
                .map(|value| {
                    value
                        ...
                        .ok_or_else(|| {
                            Error::ValidationError(
                                "texts must contain non-blank strings".to_string(),
                            )
                        })
                })
        ...
        "embedding.create_remote" => {
            ...
            let retry_count = params
                ...
                .unwrap_or(2);
            let model: Arc<dyn crate::model_inference::llm::embedding_base::EmbeddingBase> =
                Arc::new(
        ...
        "embedding.create_local" => {
            ...
            let model: Arc<dyn crate::model_inference::llm::embedding_base::EmbeddingBase> =
                Arc::new(
                    crate::model_inference::nn::queued_embedding_model::QueuedEmbeddingModel::new(
                        model_name.trim().to_string(),
                    )?,
                );
            Ok(store_script_resource(DataValue::EmbeddingModel(model)))
        ...
        "storage.create_mysql" => {
            ...
            let reference = crate::runtime::block_async(
                crate::storage::RuntimeStorageConnectionManager::shared()
                    .get_or_create_mysql_ref(&config_id),
            )?;
            Ok(store_script_resource(DataValue::RdbRef(
                crate::data_refs::RelationalDbConnection::MySql(reference),
            )))
        ...
        "storage.create_sqlite" => {
            ...
            let reference = crate::runtime::block_async(
                crate::storage::RuntimeStorageConnectionManager::shared()
                    .get_or_create_sqlite_ref(&config_id),
            )?;
            Ok(store_script_resource(DataValue::RdbRef(
                crate::data_refs::RelationalDbConnection::Sqlite(reference),
            )))
        }
        "storage.create_s3" => {
            let config_id = required_string(params, "config_id")?;
            let reference = crate::runtime::block_async(
                crate::storage::RuntimeStorageConnectionManager::shared()
                    .get_or_create_s3_ref(&config_id),
            )?;
            Ok(store_script_resource(DataValue::S3Ref(reference)))
        }
                ...
                move |pool| {
                    Box::pin(async move {
                        if let Some(group_id) = group_id {
                            sqlx::query(
                                crate::graph::message_rdb_history_common::user_history_query(Some(
                                    &group_id,
                                )),
                            )
                            .bind(sender_id)
                            .bind(group_id)
                            .bind(
                                crate::graph::message_rdb_history_common::history_query_row_limit(
                                    limit,
                                ),
                            )
                            .fetch_all(pool)
                            ...
                            sqlx::query(
                                crate::graph::message_rdb_history_common::user_history_query(None),
                            )
                            .bind(sender_id)
                            .bind(
                                crate::graph::message_rdb_history_common::history_query_row_limit(
                    ...
                    values
                        ...
                        .map(|value| {
                            value.as_f64().map(|value| value as f32).ok_or_else(|| {
                                Error::ValidationError("vector must contain numbers".to_string())
                            })
                        })
                        .collect::<Result<Vec<_>>>()
                })
        ...
        "search.query" => {
            ...
            let query = required_string(params, "query")?;
            let search_count = params
                .get("search_count")
                .and_then(Value::as_i64)
                .filter(|value| *value > 0)
                .ok_or_else(|| {
                    Error::ValidationError("search_count must be greater than 0".to_string())
                })?;
        ...
        "search.web" => {
            let engine = params
                ...
                .ok_or_else(|| {
                Error::ValidationError("web_search_engine_ref is required".to_string())
            })?;
            ...
            let results = if !url.is_empty() {
                ...
                match engine.search(&query, count) {
                    Ok(results) => results,
                    Err(_error) if reqwest::Url::parse(&query).is_ok() => {
                        engine.fetch_url_direct(&query)?
                    }
