            ...
            &mut |method, params| match method {
                    ...
                    Ok(variables
                        ...
                        .and_then(|store| {
                            store
                                .read()
                                .ok()
                                .and_then(|values| values.get(name).map(script_value_to_json))
                        })
                        .unwrap_or(Value::Null))
                }
                "variables.set" => {
                    let name = params.get("name").and_then(Value::as_str).ok_or_else(|| {
                        Error::ValidationError("variables.set 缺少 name".to_string())
                    })?;
                    ...
                    let parsed = super::registry::json_to_data_value(&value, &super::DataType::Any)
                        .ok_or_else(|| {
                            Error::ValidationError("variables.set value 无效".to_string())
                        })?;
                    store
                        .write()
                ...
                "task.progress" => {
                    let message = params
                        ...
                        .ok_or_else(|| {
                            Error::ValidationError("task.progress 缺少 message".to_string())
                        })?;
                ...
                "bot.send" => {
                    ...
                    let sender =
                        ...
                        >(params.get("sender").cloned().ok_or_else(
                            || Error::ValidationError("sender is required".to_string()),
                        )?)
                        .map_err(|error| {
                            Error::ValidationError(format!("sender is invalid: {error}"))
                        })?;
                    let messages = serde_json::from_value::<
                        Vec<crate::ims_bot_adapter::models::message::Message>,
                    >(
                        params.get("message").cloned().ok_or_else(|| {
                            Error::ValidationError("message is required".to_string())
                        })?,
                    )
                    .map_err(|error| {
                    ...
                    let success =
                        crate::ims_bot_adapter::runtime::ws_action::response_success(&response);
                    let message_id =
                ...
                "bot.filter_event_type" => {
                    let event = message_event(params)?;
                    let filter_type = optional_string(params, "filter_type")
                ...
                "session.clear" => {
                    ...
                    let cleared = if let Ok(handle) = tokio::runtime::Handle::try_current() {
                        ...
                        tokio::runtime::Runtime::new()?
                            .block_on(session_ref.clear_state(&sender_id))
                    };
                ...
                "session.release" => {
                    ...
                    let released = if let Ok(handle) = tokio::runtime::Handle::try_current() {
                        block_in_place(|| {
                            handle.block_on(session_ref.release(&sender_id, claim_token))
                        })
                    } else {
                        tokio::runtime::Runtime::new()?
                ...
                "message_cache.append" => {
                    ...
                    let messages = llm_messages_from_json(
                        params.get("messages").ok_or_else(|| {
                            Error::ValidationError("messages is required".to_string())
                        })?,
                        "messages",
                    )?;
                ...
                "message_cache.clear" => {
                    ...
                    let cleared = if let Ok(handle) = tokio::runtime::Handle::try_current() {
                        ...
                        tokio::runtime::Runtime::new()?
                            .block_on(cache_ref.clear_messages(&sender_id))?
                    };
                    Ok(Value::Bool(cleared))
                }
                ...
                "embedding.batch_infer" => {
                    ...
                    let texts = params
                        ...
                        .ok_or_else(|| {
                            Error::ValidationError("texts must be Vec<String>".to_string())
                        })?
                        .iter()
                        .map(|value| {
                            value
                ...
                "storage.create_redis" => {
                    let config_id = required_string(params, "config_id")?;
                    let reference =
                        crate::storage::redis::build_redis_ref_for_connection(&config_id)?;
                ...
                "storage.search_messages" => {
                    ...
                    let builder =
                        crate::graph::message_rdb_history_common::SearchMessagesQueryBuilder {
                ...
                "storage.persist_qq_message_rdb" => {
                    ...
                    let messages = qq_messages_param(params, "qq_message_list")?;
                    let success =
                        crate::graph::qq_message_list_rdb_persistence::persist_qq_message_list(
                            &rdb_ref,
                            &messages,
                            required_string(params, "message_id")?,
                            required_string(params, "sender_id")?,
                            ...
                            values
                                ...
                                .map(|value| {
                                    value.as_f64().map(|value| value as f32).ok_or_else(|| {
                                        Error::ValidationError(
                                            "vector must contain numbers".to_string(),
                                        )
                                    })
                                })
                                .collect::<Result<Vec<_>>>()
                ...
                "storage.search_images" => {
                    ...
                    let limit = params
                        ...
                        .filter(|value| *value > 0)
                        .ok_or_else(|| {
                            Error::ValidationError("limit must be greater than 0".to_string())
                        })? as usize;
