...
impl SubAgentDefinition {
    pub fn validate(&self, available_tool_ids: &HashSet<String>) -> Result<()> {
        ...
        if self.name.trim().is_empty() {
            return Err(Error::ValidationError("subagent name must not be empty".to_string()));
        }
        if available_tool_ids.contains(&self.id) {
            return Err(Error::ValidationError(format!(
                "subagent id '{}' conflicts with an available tool id",
                self.id
            )));
        }
        ...
        for tool_id in &self.tool_ids {
            if tool_id.trim().is_empty() || !seen.insert(tool_id) {
                return Err(Error::ValidationError(format!(
                    "subagent '{}' has duplicate or empty tool id",
                    self.id
                )));
            }
            if !available_tool_ids.contains(tool_id) {
                return Err(Error::ValidationError(format!(
                    "subagent '{}' is not allowed to use tool '{tool_id}'",
                    self.id
                )));
            }
        }
...
fn validate_ports(kind: &str, ports: &[FunctionPortDef]) -> Result<()> {
    ...
    for port in ports {
        ...
        if name.is_empty() || !names.insert(name) {
            return Err(Error::ValidationError(format!(
                "subagent {kind} ports must have unique non-empty names"
            )));
        }
    }
    Ok(())
...
impl SubAgent {
    ...
    pub fn definition(&self) -> &SubAgentDefinition {
        &self.definition
    }
}
    ...
    async fn run(&self, _context: AgentContext, input: Self::Input) -> Result<Self::Output> {
        for port in &self.definition.inputs {
            ...
            if port.required && value.is_none() {
                return Err(Error::ValidationError(format!(
                    "subagent '{}' missing required input '{}'",
        ...
        let (messages, stop_reason) = engine.run(vec![
        ...
        if !matches!(stop_reason, ToolCallingStopReason::Done) {
            return Err(Error::ValidationError(format!(
                "subagent '{}' did not complete normally: {stop_reason:?}",
                self.definition.id
            )));
        }
        let text = messages.last().and_then(LLMMessage::content_text_owned).ok_or_else(|| {
            Error::ValidationError(format!("subagent '{}' returned no text", self.definition.id))
        })?;
        let output: Map<String, Value> = serde_json::from_str(text.trim()).map_err(|error| {
            Error::ValidationError(format!(
                "subagent '{}' returned invalid output JSON: {error}",
                self.definition.id
            ))
        })?;
        ...
        for port in &self.definition.outputs {
            ...
            if port.required && value.is_none() {
                return Err(Error::ValidationError(format!(
                    "subagent '{}' missing required output '{}'",
                    self.definition.id, port.name
                )));
            }
...
fn subagent_input_from_tool_arguments(
    inputs: &[FunctionPortDef],
    arguments: &Value,
) -> Result<HashMap<String, DataValue>> {
    let object = arguments.as_object().ok_or_else(|| {
        Error::ValidationError("subagent arguments must be an object".to_string())
    })?;
    ...
    for port in inputs {
        if let Some(value) = object.get(&port.name) {
            ...
            return Err(Error::ValidationError(format!(
                "missing required subagent tool argument '{}'",
                port.name
            )));
        }
    }
...
impl FunctionTool for SubAgentToolSpec {
    ...
    fn parameters(&self) -> Value {
        json!({"type":"object", "properties": self.definition.inputs.iter().map(|port| (port.name.clone(), json!({"description":port.description}))).collect::<Map<_, _>>(), "required": self.definition.inputs.iter().filter(|port| port.required).map(|port| port.name.clone()).collect::<Vec<_>>(), "additionalProperties":false})
    }
...
mod tests {
    ...
    fn tool_arguments_follow_declared_input_ports() {
        ...
        let input =
            subagent_input_from_tool_arguments(&inputs, &json!({ "content": "hello" })).unwrap();
