...
fn parse_status(value: String) -> Result<ScheduledTaskStatus> { match value.as_str() { "pending" => Ok(ScheduledTaskStatus::Pending), "running" => Ok(ScheduledTaskStatus::Running), "succeeded" => Ok(ScheduledTaskStatus::Succeeded), "failed" => Ok(ScheduledTaskStatus::Failed), "cancelled" => Ok(ScheduledTaskStatus::Cancelled), _ => Err(crate::string_error!("unknown scheduled task status '{value}'")) } }
fn related(row: &sqlx::mysql::MySqlRow) -> Result<Vec<String>> { serde_json::from_str(&row.try_get::<Option<String>, _>("related_task_ids").map_err(Error::Database)?.unwrap_or_else(|| "[]".to_string())).map_err(|err| crate::string_error!("parse related task ids: {err}")) }
