...
use crate::inference::nn::embedding::embedding_runtime_manager::RuntimeEmbeddingModelManager;
use crate::inference::nodes::llm_node::build_llm;
use crate::inference::system_config::{load_llm_refs, ModelRefSpec};
...
pub fn build_embedding_from_ref_id(embedding_model_ref_id: Option<&str>) -> Result<Arc<dyn EmbeddingBase>> {
    let embedding_model_ref_id = embedding_model_ref_id
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| crate::error::Error::ValidationError("embedding_model_ref_id is required".to_string()))?;
