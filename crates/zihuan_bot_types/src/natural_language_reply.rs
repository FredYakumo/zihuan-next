...
mod tests {
    use super::{
        json_to_qq_message_vec, json_value_to_qq_message_vec, qq_message_json_output_system_prompt,
    };
    use crate::message::Message;
    use zihuan_core::error::Result;

    #[test]
    fn system_prompt_keeps_json_rules_only() {
        let prompt = qq_message_json_output_system_prompt();

        assert!(prompt.contains("只输出纯 JSON 数组"));
        assert!(prompt.contains("二维 JSON 数组"));
        assert!(prompt.contains("外层数组表示总共要发送的多条消息"));
        assert!(prompt.contains("\"message_type\":\"plain_text\""));
        assert!(prompt.contains("\"message_type\":\"at\""));
        assert!(prompt.contains("\"message_type\":\"combine_text\""));
        assert!(!prompt.contains("角色\u{201c}"));
        assert!(!prompt.contains("语言风格"));
        assert!(!prompt.contains("目标 ID"));
        assert!(!prompt.contains("群聊定向回复补充规则"));
        assert!(!prompt.contains("max_one_reply_length"));
    }

    #[test]
    fn parses_plain_text_batch() -> Result<()> {
        let messages =
            json_to_qq_message_vec(r#"[[{"message_type":"plain_text","content":"你好"}]]"#)?;

        assert!(matches!(
            messages.as_slice(),
            [batch] if matches!(
                batch.as_slice(),
                [Message::PlainText(text)] if text.text == "你好"
            )
        ));
        Ok(())
    }
    ...
    #[test]
    fn parses_top_level_at_message() -> Result<()> {
        let messages = json_to_qq_message_vec(r#"[[{"message_type":"at","target":"42"}]]"#)?;

        assert!(matches!(
            messages.as_slice(),
            [batch] if matches!(
                batch.as_slice(),
                [Message::At(at)] if at.target.as_deref() == Some("42")
            )
        ));
        Ok(())
    }
    ...
    #[test]
    fn flattens_combine_text_into_message_vec_in_same_batch() -> Result<()> {
        let messages = json_to_qq_message_vec(
            r#"[[{"message_type":"combine_text","content_list":[{"message_type":"at","target":"42"},{"message_type":"plain_text","content":" "},{"message_type":"plain_text","content":"你好"}]}]]"#,
        )?;
    ...
    #[test]
    fn parses_multiple_batches() -> Result<()> {
        let messages = json_to_qq_message_vec(
            r#"[[{"message_type":"plain_text","content":"第一条"}],[{"message_type":"plain_text","content":"第二条"}]]"#,
        )?;
    ...
    fn rejects_empty_array() {
        let err = json_to_qq_message_vec("[]").expect_err("empty array should be rejected");
        assert!(err.to_string().contains("must not be empty"));
    }

    #[test]
    fn rejects_empty_inner_batch() {
        let err = json_to_qq_message_vec("[[]]").expect_err("empty batch should be rejected");
        assert!(err.to_string().contains("batch 1"));
    }

    #[test]
    fn rejects_legacy_one_dimensional_array() {
        let err = json_to_qq_message_vec(r#"[{"message_type":"plain_text","content":"你好"}]"#)
            .expect_err("legacy one-dimensional format should be rejected");
        assert!(!err.to_string().is_empty());
    }

    #[test]
    fn rejects_blank_top_level_plain_text() {
        let err = json_to_qq_message_vec(r#"[[{"message_type":"plain_text","content":"   "}]]"#)
            .expect_err("blank plain_text should be rejected");
        assert!(err.to_string().contains("must not be blank"));
    }

    #[test]
    fn rejects_empty_at_target() {
        let err = json_to_qq_message_vec(r#"[[{"message_type":"at","target":"   "}]]"#)
            .expect_err("blank at target should be rejected");
        assert!(err.to_string().contains("at.target"));
    }
    ...
    #[test]
    fn rejects_combine_text_without_substantive_text() {
        let err = json_to_qq_message_vec(
            r#"[[{"message_type":"combine_text","content_list":[{"message_type":"at","target":"42"},{"message_type":"plain_text","content":" "}]}]]"#,
        )
        .expect_err("combine_text without substantive text should be rejected");
        assert!(err.to_string().contains("substantive plain_text"));
    }

    #[test]
    fn rejects_invalid_json() {
        assert!(json_to_qq_message_vec("not json").is_err());
    }

    #[test]
    fn parses_json_value_directly() -> Result<()> {
        let messages = json_value_to_qq_message_vec(&serde_json::json!([
