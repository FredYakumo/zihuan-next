...
impl NodeGraph {
    ...
    fn spawn_event_task_with_edges(
        ...
        input_sources: &InputSourceMap,
        task_error: Arc<Mutex<Option<zihuan_core::error::Error>>>,
    ) -> Result<()> {
    ...
    fn run_event_producer_with_edges(
        ...
        let task_error: Arc<Mutex<Option<zihuan_core::error::Error>>> = Arc::new(Mutex::new(None));
        {
            let inputs = {
                let node = self.nodes.get(node_id).ok_or_else(|| {
                    zihuan_core::error::Error::ValidationError(format!(
...
mod tests {
    ...
    fn graph_execution_resets_runtime_variables_to_initial_values_each_run() -> Result<()> {
        let definition = NodeGraphDefinition {
            ...
            hyperparameter_groups: Vec::new(),
            hyperparameters: Vec::new(),
            variables: vec![crate::graph_io::GraphVariable {
