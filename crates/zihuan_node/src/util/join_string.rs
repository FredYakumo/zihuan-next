...
#[cfg(test)]
mod tests {
    use super::JoinStringNode;
    use crate::{DataType, DataValue, Node};
    use std::collections::HashMap;

    #[test]
    fn joins_string_vector_with_delimiter() {
        let mut node = JoinStringNode::new("join", "Join");
            ...
            (
                ...
                DataValue::Vec(
                    ...
                    vec![
                        ...
                        DataValue::String("gamma".to_string()),
                    ],
                ),
            ),
        ...
        assert!(matches!(
            outputs.get("result"),
            Some(DataValue::String(value)) if value == "alpha, beta, gamma"
        ));
    }

    #[test]
    fn joins_empty_vector_to_empty_string() {
        let mut node = JoinStringNode::new("join", "Join");
        let inputs = HashMap::from([
            (
                "strings".to_string(),
