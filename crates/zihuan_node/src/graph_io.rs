...
mod tests {
    ...
    fn ensure_registry_initialized() {
        ...
        INIT.call_once(|| {
            let _ = crate::registry::init_node_registry();
            // Register a minimal stub for "brain" so graph validation tests can run
            // without the main binary's LLM module.
            use crate::brain_tool_spec::{
                brain_shared_inputs_from_value, BRAIN_SHARED_INPUTS_PORT, BRAIN_TOOLS_CONFIG_PORT,
            };
            struct StubBrainNode {
                id: String,
                name: String,
                extra_inputs: Vec<crate::Port>,
            }
            impl StubBrainNode {
            ...
            impl crate::Node for StubBrainNode {
                ...
                fn apply_inline_config(
                    ...
                    match inline_values.get(BRAIN_SHARED_INPUTS_PORT) {
                        Some(crate::DataValue::Json(value)) if !value.is_null() => {
                            if let Some(shared_inputs) = brain_shared_inputs_from_value(value) {
                                self.extra_inputs = shared_inputs
                                    .into_iter()
                                    .map(|p| crate::Port::new(&p.name, p.data_type))
                                    .collect();
                            }
                        }
                        _ => {
    ...
    fn refresh_port_types_syncs_brain_tool_subgraph_with_shared_inputs() {
        ...
        let input_boundary = tools[0]
            ...
            .nodes
            .iter()
            .find(|node| node.id == crate::function_graph::FUNCTION_INPUTS_NODE_ID)
            .expect("function_inputs");
        let signature = crate::function_graph::function_signature_from_inline_values(
