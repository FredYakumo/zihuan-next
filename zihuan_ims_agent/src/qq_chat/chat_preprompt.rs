...
use zihuan_core::agent_runtime::brain::{Brain, BrainStopReason, BrainTool, ToolExecutionOutput, ToolRunDuration};
...
use zihuan_core::graph::brain_tool_spec::{brain_tool_input_signature, BrainToolDefinition, ToolParamDef};
use zihuan_core::graph::function_graph::{
    sync_function_subgraph_signature, FUNCTION_INPUTS_NODE_ID, FUNCTION_OUTPUTS_NODE_ID,
};
use zihuan_core::graph::graph_io::refresh_port_types;
use zihuan_core::graph::registry::build_node_graph_from_definition;
use zihuan_core::graph::util::function::{
...
impl DreamNodeGraphTool {
    ...
    fn run_node_graph(&self, call_content: &str, arguments: &Value) -> zihuan_core::error::Result<String> {
        ...
        for parameter in &self.definition.parameters {
            ...
            let port = zihuan_core::graph::function_graph::FunctionPortDef {
        ...
        function_inputs_node.inline_values.insert(
            zihuan_core::graph::function_graph::FUNCTION_SIGNATURE_PORT.to_string(),
...
pub(crate) fn run_chat_preprompt_agent(
    ...
    let dream_memory = rdb_pool.as_ref().and_then(|connection| {
        match block_async(zihuan_core::scheduled_task::latest_dream_memory(connection, agent_id, sender_id)) {
            Ok(memory) => memory,
            Err(err) => {
