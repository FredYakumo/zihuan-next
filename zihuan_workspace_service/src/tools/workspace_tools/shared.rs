...
pub(crate) fn glob_matches(path: &Path, root: &Path, glob: Option<&str>) -> bool {
    ...
    glob.split(',').map(str::trim).filter(|value| !value.is_empty()).all(|pattern| {
        let (exclude, pattern) =
            pattern.strip_prefix('!').map_or((false, pattern), |value| (true, value));
...
pub(crate) fn collect_search_files(
    ...
    while let Some(path) = pending.pop_front() {
        ...
        for entry in fs::read_dir(&path)
            .map_err(|err| format!("failed to list directory '{}': {err}", path.display()))?
        {
            let entry = entry.map_err(|err| {
                format!("failed to inspect directory '{}': {err}", path.display())
            })?;
...
pub(crate) fn execute_search(
    ...
    let regex_pattern = if args.case_sensitive {
        ...
        format!("(?i){}", regex::escape(&regex_source))
    };
    ...
    if patterns.is_empty() {
        return json_error("pattern must not be empty");
    }
    ...
    let mut values: Vec<(String, SearchMatch)> = matches
        .into_iter()
        .map(|item| {
                ...
                source
                    ...
                    .map(|matched| {
                        if let Some(template) = args.output.as_deref() {
                            render_capture_template(
                                template,
                                source.captures(matched.as_str()).as_ref(),
                            )
                        } else {
    ...
    success_json(
        serde_json::json!({ "ok": true, "path": path.display().to_string(), "pattern": pattern,
        ...
        if chars[index] == '$' {
            ...
            if end > start {
                ...
                let capture = key
                    .parse::<usize>()
                    .ok()
                    .and_then(|number| captures.get(number).map(|value| value.as_str().to_string()))
                    .or_else(|| captures.name(&key).map(|value| value.as_str().to_string()));
