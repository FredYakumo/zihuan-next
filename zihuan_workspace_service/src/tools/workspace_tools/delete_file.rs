...
impl Tool for DeleteFileTool {
    fn spec(&self) -> Arc<dyn FunctionTool> {
        Arc::new(StaticFunctionToolSpec {
            ...
            parameters: serde_json::json!({"type":"object","properties":{"path":{"type":"string"},"recursive":{"type":"boolean"},"missing_ok":{"type":"boolean"}},"required":["path"]}),
        })
    }
    fn execute(&self, _: &str, a: &Value) -> String {
        let args: DeleteFileArgs = match serde_json::from_value(a.clone()) {
            Ok(v) => v,
            Err(e) => return json_error(format!("invalid delete_file arguments: {e}")),
        };
        ...
        if !path.exists() {
            if args.missing_ok {
                return success_json(
                    serde_json::json!({"ok":true,"path":path.display().to_string(),"deleted":false}),
                );
            }
        ...
        let result = if path.is_dir() {
            if !args.recursive {
                return json_error("recursive=true is required to delete a directory");
            }
        ...
        match result {
            Ok(()) => success_json(
                serde_json::json!({"ok":true,"path":path.display().to_string(),"deleted":true,"line_count":line_count}),
            ),
            Err(e) => json_error(format!("failed to delete '{}': {e}", path.display())),
        }
    }
    fn execution_resource(&self, a: &Value) -> ToolExecutionResource {
        serde_json::from_value::<DeleteFileArgs>(a.clone())
            .map(|v| path_resource(self.workspace_path.as_deref(), &v.path, true))
