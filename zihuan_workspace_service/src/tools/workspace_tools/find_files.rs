...
impl Tool for FindFilesTool {
    ...
    fn execute(&self, _: &str, arguments: &Value) -> String {
        ...
        if args.name.is_none() && args.glob.is_none() {
            return json_error("name or glob is required");
        }
        let kind = args.kind.as_deref().unwrap_or("all");
        if !matches!(kind, "file" | "dir" | "all") {
            return json_error("type must be file, dir, or all");
        }
        let root = match resolve_tool_path(self.workspace_path.as_deref(), &args.path) {
            Ok(path) => path,
            Err(err) => return json_error(err.to_string()),
        };
        if !root.is_dir() {
            return json_error(format!("directory does not exist: {}", root.display()));
        }
        ...
        while let Some(directory) = pending.pop_front() {
            ...
            for entry in children {
                ...
                let excluded = args.exclude.iter().any(|pattern| {
                    let pattern =
                        pattern.trim_start_matches('!').trim_matches('/').replace('\\', "/");
                    let matched =
                        wildcard_matches(&pattern, &name) || wildcard_matches(&pattern, &relative);
                ...
                let type_match =
                    kind == "all" || (kind == "dir" && is_dir) || (kind == "file" && !is_dir);
                let name_match =
                    args.name.as_deref().is_none_or(|pattern| wildcard_matches(pattern, &name));
                let glob_match = args.glob.as_deref().is_none_or(|pattern| {
                    wildcard_matches(pattern, &relative) || wildcard_matches(pattern, &name)
                });
        ...
        success_json(
            serde_json::json!({"ok":true,"path":root.display().to_string(),"matches":results,"total_matches":results.len(),"truncated":truncated}),
        )
    }

    fn execution_resource(&self, arguments: &Value) -> ToolExecutionResource {
        serde_json::from_value::<FindFilesArgs>(arguments.clone())
