...
impl Tool for FileInfoTool {
    fn spec(&self) -> Arc<dyn FunctionTool> {
        Arc::new(StaticFunctionToolSpec {
            name: DEFAULT_TOOL_FILE_INFO,
            description: "Return file, directory, and symlink metadata",
            parameters: serde_json::json!({"type":"object","properties":{"path":{"type":"string"}},"required":["path"]}),
        })
    }

    fn execute(&self, _: &str, arguments: &Value) -> String {
        let args: FileInfoArgs = match serde_json::from_value(arguments.clone()) {
            Ok(value) => value,
            Err(err) => return json_error(format!("invalid file_info arguments: {err}")),
        };
        ...
        let metadata = match fs::metadata(&path) {
            Ok(value) => value,
            Err(err) => {
                return json_error(format!("failed to inspect '{}': {err}", path.display()))
            }
        };
        ...
        if metadata.is_dir() {
            ...
            return success_json(
                serde_json::json!({"ok":true,"path":path.display().to_string(),"type":"directory","size_bytes":metadata.len(),"line_count":null,"modified_unix_secs":modified,"is_binary":false,"is_symlink":symlink,"entry_count":entry_count,"extensions":extensions}),
            );
        }
        let bytes = match fs::read(&path) {
            Ok(value) => value,
            Err(err) => return json_error(format!("failed to read '{}': {err}", path.display())),
        };
        ...
        success_json(
            serde_json::json!({"ok":true,"path":path.display().to_string(),"type":"file","size_bytes":metadata.len(),"line_count":line_count,"modified_unix_secs":modified,"is_binary":is_binary,"is_symlink":symlink}),
        )
    }

    fn execution_resource(&self, arguments: &Value) -> ToolExecutionResource {
        serde_json::from_value::<FileInfoArgs>(arguments.clone())
