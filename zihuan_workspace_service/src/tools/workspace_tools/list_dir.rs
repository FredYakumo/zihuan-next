use super::shared::{
    json_error, path_resource, resolve_tool_path, success_json, wildcard_matches,
    DEFAULT_MAX_ENTRIES,
};
...
impl Tool for ListDirTool {
    fn spec(&self) -> Arc<dyn FunctionTool> {
        Arc::new(StaticFunctionToolSpec {
            ...
            description:
                "List files and directories, optionally filtered by name and rendered as a tree",
            parameters: serde_json::json!({"type":"object","properties":{"path":{"type":"string"},"recursive":{"type":"boolean"},"include_hidden":{"type":"boolean"},"max_entries":{"type":"integer","minimum":1},"name_glob":{"type":"string"},"format":{"type":"string","enum":["json","tree"]}},"required":["path"]}),
        })
    }
    fn execute(&self, _: &str, arguments: &Value) -> String {
        ...
        while let Some(directory) = pending.pop_front() {
            ...
            for entry in match fs::read_dir(&directory) {
                ...
                if let Some(pattern) = args.name_glob.as_deref() {
                    let included =
                        pattern.split(',').map(str::trim).filter(|value| !value.is_empty()).all(
                            |pattern| {
                                let (exclude, pattern) = pattern
                                    .strip_prefix('!')
                                    .map_or((false, pattern), |value| (true, value));
        ...
        let tree = entries
            ...
            .map(|entry| {
                ...
                let relative = std::path::Path::new(entry_path)
                    .strip_prefix(&path)
                    .unwrap_or(std::path::Path::new(entry_path))
                    .to_string_lossy();
            ...
            .join("\n");
        success_json(
            serde_json::json!({"ok":true,"path":path.display().to_string(),"recursive":args.recursive,"entries":entries,"tree":tree,"format":args.format.as_deref().unwrap_or("json"),"truncated":truncated}),
        )
    }
