...
fn temp_dir() -> PathBuf {
    ...
    let path =
        std::env::temp_dir().join(format!("zihuan-git-status-{}-{suffix}", std::process::id()));
...
fn git_status_returns_branch_and_worktree_changes_without_a_shell() {
    ...
    let init = Command::new("git")
        .args(["-C", directory.to_str().unwrap(), "init", "-q"])
        .output()
        .unwrap();
    assert!(
        init.status.success(),
    ...
    let result =
        serde_json::from_str::<serde_json::Value>(&tool.execute("", &json!({"path":"."}))).unwrap();
    assert_eq!(result["ok"], true);
    assert!(
        result["branch"].as_str().unwrap().contains("No commits yet")
            || result["branch"].as_str().unwrap().contains("HEAD")
    );
    assert!(result["changes"]
        .as_array()
        .unwrap()
        .iter()
        .any(|item| item["path"] == "changed.txt"));
