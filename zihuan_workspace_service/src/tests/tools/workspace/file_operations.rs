...
use crate::tools::workspace_tools::{
    CopyFileTool, FileInfoTool, MoveFileTool, DEFAULT_TOOL_COPY_FILE, DEFAULT_TOOL_FILE_INFO,
    DEFAULT_TOOL_MOVE_FILE,
};

fn temp_dir() -> PathBuf {
    let suffix = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos();
    let path =
        std::env::temp_dir().join(format!("zihuan-file-ops-{}-{suffix}", std::process::id()));
...
fn copy_move_and_file_info_support_common_file_workflow() {
    ...
    let copied = serde_json::from_str::<serde_json::Value>(
        &copy.execute("", &json!({"src":"source.txt","dest":"backup.txt"})),
    )
    .unwrap();
    assert_eq!(copied["ok"], true);
    let moved = serde_json::from_str::<serde_json::Value>(
        &move_tool.execute("", &json!({"src":"backup.txt","dest":"renamed.txt"})),
    )
    .unwrap();
    assert_eq!(moved["ok"], true);
    let metadata = serde_json::from_str::<serde_json::Value>(
        &info.execute("", &json!({"path":"renamed.txt"})),
    )
    .unwrap();
