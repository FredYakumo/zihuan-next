...
/// Multimodal content for an `OpenAIMessage`.
///
/// Mirrors the Kimi / OpenAI vision content spec: either a plain string
/// (`"content": "hi"`) or an array of typed parts
/// (`"content": [{"type":"text",...},{"type":"image_url",...}]`).
///
/// `#[serde(untagged)]` keeps backward compatibility with existing
/// persisted histories that store `content` as a bare string.
...
/// Kimi accepts `image_url` / `video_url` either as a bare string or as
/// an object with a `url` field. Both forms round-trip via `untagged`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
impl OpenAIMessage {
    ...
    /// Borrowed view of the textual content. Returns `Some` only when the
    /// content is the plain-text variant; multimodal `Parts` returns `None`.
    ...
    /// Owned textual view. For multimodal `Parts`, all `Text` segments are
    /// joined with newlines so legacy callers still get a usable string.
