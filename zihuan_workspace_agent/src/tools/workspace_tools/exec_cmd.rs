...
impl BrainTool for ExecCmdBrainTool{
 ...
 fn execute_with_progress(&self, _:&str, a:&Value, on_output: Arc<dyn Fn(&str, &str) + Send + Sync>)->zihuan_core::agent::brain::ToolExecutionOutput{
  ...
  let shell=args.shell.clone().unwrap_or_else(||if cfg!(windows){"powershell".to_string()}else{"bash".to_string()});
      ...
      if !is_approved(self.session_id.as_deref(),&args.command){let Some(session_id)=self.session_id.as_deref() else{return zihuan_core::agent::brain::ToolExecutionOutput::text(json_error("exec_cmd requires a chat session"))};{let (lock,_)=COMMAND_APPROVALS.get_or_init(||(Mutex::new(CommandApprovals::default()),Condvar::new()));lock.lock().unwrap().pending.insert(session_id.to_string(),PendingCommandApproval{command:args.command.clone(),shell:shell.clone()});}(on_output)("command_confirmation",&serde_json::json!({"command":args.command,"shell":shell}).to_string());if !wait_for_command_decision(session_id,&args.command){return zihuan_core::agent::brain::ToolExecutionOutput::text(serde_json::json!({"ok":false,"rejected":true,"error":"command execution rejected by user"}).to_string())}}
