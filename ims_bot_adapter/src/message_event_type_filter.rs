...
impl Node for MessageEventTypeFilterNode {
    ...
    node_output![
        port! { name = "true_event",  ty = crate::models::event_model::MessageEvent, desc = "消息类型匹配时的输出" },
        port! { name = "false_event", ty = crate::models::event_model::MessageEvent, desc = "消息类型不匹配时的输出" },
    ];
