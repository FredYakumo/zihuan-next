...
fn image_part_from_bytes_with_mime(mime_type: &str, bytes: Vec<u8>) -> LLMMessagePart {
    let base64_payload = base64::engine::general_purpose::STANDARD.encode(bytes);
    LLMMessagePart::image_data_url(mime_type, base64_payload)
}

fn image_part_from_bytes(image: &ImageMessage, bytes: Vec<u8>) -> LLMMessagePart {
...
fn resolve_remote_url_as_image_part(
    ...
    if url.starts_with("data:") {
        return Some(ResolvedImagePart {
            part: LLMMessagePart::image_url_string(url.to_string()),
