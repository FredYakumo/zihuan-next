...
impl TempDir {
    fn new() -> Self {
        let path =
            std::env::temp_dir().join(format!("zihuan-agents-md-test-{}", uuid::Uuid::new_v4()));
    ...
    fn write_agents_md(&self, content: &str) {
        std::fs::write(self.agents_md_path(), content).expect("write AGENTS.md");
    }
}
...
fn system_prompt(
    provider: &Arc<dyn InferenceToolProvider>,
    workspace_path: Option<String>,
) -> Option<String> {
    let mut messages = vec![LLMMessage::user("hello")];
    provider.augment_messages(
        &mut messages,
...
fn with_home<F, R>(home: &std::path::Path, action: F) -> R
where
    F: FnOnce() -> R,
{
...
fn agents_md_enabled_workspace_file_references_agents_md() {
    ...
    let prompt = system_prompt(&provider(true), Some(workspace.path.to_string_lossy().to_string()))
        .expect("system prompt");
    let expected =
        std::fs::canonicalize(workspace.agents_md_path()).expect("canonical workspace AGENTS.md");
...
fn agents_md_disabled_does_not_reference_agents_md() {
    ...
    workspace.write_agents_md("# Project rules");
    let prompt =
        system_prompt(&provider(false), Some(workspace.path.to_string_lossy().to_string()))
            .expect("system prompt");
    assert!(!prompt.contains("AGENTS.md"));
    assert!(prompt.contains("workspace directory"));
}
...
fn agents_md_enabled_missing_file_does_not_reference_agents_md() {
    let workspace = TempDir::new();
    let prompt = with_home(&workspace.path, || {
        system_prompt(&provider(true), Some(workspace.path.to_string_lossy().to_string()))
    })
    .expect("system prompt");
    assert!(!prompt.contains("AGENTS.md"));
}
...
fn agents_md_enabled_home_file_references_agents_md() {
    ...
    let workspace = TempDir::new();
    let prompt = with_home(&home.path, || {
        system_prompt(&provider(true), Some(workspace.path.to_string_lossy().to_string()))
    })
    .expect("system prompt");
...
fn agents_md_enabled_multiple_files_lists_in_priority_order() {
    ...
    let prompt = with_home(&home.path, || system_prompt(&provider(true), Some(workspace_path)))
        .expect("system prompt");
    let workspace_line = format!(
        "- {}",
        std::fs::canonicalize(workspace.agents_md_path())
            .expect("canonical workspace")
            .display()
    );
    let home_line = format!(
        "- {}",
        std::fs::canonicalize(home.agents_md_path()).expect("canonical home").display()
    );
    assert!(
        prompt.find(&workspace_line).expect("workspace line")
            < prompt.find(&home_line).expect("home line")
    );
}
