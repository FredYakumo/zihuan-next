...
pub struct WorkflowInfo {
    ...
    pub inputs: Vec<zihuan_core::graph::function_graph::FunctionPortDef>,
    pub outputs: Vec<zihuan_core::graph::function_graph::FunctionPortDef>,
}
    ...
    let mut workflows: Vec<WorkflowInfo> = entries
        ...
        .filter_map(|e| {
            ...
            let (display_name, description, version, inputs, outputs) =
                zihuan_core::graph::load_graph_definition_from_json(&p)
                    .ok()
                    .map(|graph| {
                        (
                            graph.metadata.name,
...
pub async fn open_file(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let result = zihuan_core::graph::load_graph_definition_from_json(&body.path);
    match result {
        Ok(mut graph) => {
