...
use zihuan_core::model_inference::system_config::{load_agents, AgentConfig, AgentToolType, NodeGraphToolConfig};
use salvo::prelude::*;
use salvo::writing::Json;
use serde::{Deserialize, Serialize};
use zihuan_core::storage_handler::{load_connections, ObjectStorageConfig};
...
pub struct WorkflowInfo {
    ...
    pub inputs: Vec<zihuan_core::graph_engine::function_graph::FunctionPortDef>,
    pub outputs: Vec<zihuan_core::graph_engine::function_graph::FunctionPortDef>,
}
    ...
    let mut workflows: Vec<WorkflowInfo> = entries
        ...
        .filter_map(|e| {
            ...
            let (display_name, description, version, inputs, outputs) =
                zihuan_core::graph_engine::load_graph_definition_from_json(&p)
                    .ok()
                    .map(|graph| {
                        (
                            graph.metadata.name,
...
async fn hot_reload_agents_for_saved_graph(state: Arc<AppState>, broadcast_tx: WsBroadcast, saved_path: &str) {
        ...
        .filter(|agent| agent_uses_saved_graph(agent, &normalized_saved_path, saved_workflow_name))
    {
        ...
        if runtime.status != zihuan_core::agent_runtime::AgentRuntimeStatus::Running {
            continue;
        }
...
pub async fn open_file(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let result = zihuan_core::graph_engine::load_graph_definition_from_json(&body.path);
    match result {
        Ok(mut graph) => {
            zihuan_core::graph_engine::graph_boundary::sync_root_graph_io(&mut graph);
            zihuan_core::graph_engine::ensure_positions(&mut graph);
...
pub async fn upload_graph(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let graph: zihuan_core::graph_engine::graph_io::NodeGraphDefinition = match serde_json::from_slice(&body_bytes) {
        Ok(v) => v,
        Err(e) => {
            res.status_code(StatusCode::UNPROCESSABLE_ENTITY);
            res.render(Json(serde_json::json!({"error": e.to_string()})));
