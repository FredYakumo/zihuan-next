...
fn validate_connection(
    ...
    let ConnectionKind::Weaviate(weaviate) = kind else {
        if let ConnectionKind::Elasticsearch(elasticsearch) = kind {
            let reference = zihuan_core::storage_handler::ElasticsearchRef::new(elasticsearch.clone())
                .map_err(|err| ConnectionValidationError::BadRequest(err.to_string()))?;
            return zihuan_core::storage_handler::ensure_elasticsearch_index(&reference, allow_create_collection)
                .map_err(|err| ConnectionValidationError::BadRequest(err.to_string()));
        }
        return Ok(false);
    };
    ...
    .map_err(|err| ConnectionValidationError::BadRequest(err.to_string()))?;
    let result =
        zihuan_core::storage_handler::ensure_collection_schema(&weaviate_ref, weaviate.collection_schema, allow_create_collection)
            .map_err(|err| ConnectionValidationError::BadRequest(err.to_string()))?;
