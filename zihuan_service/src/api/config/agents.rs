...
use log::{info, warn};
use crate::agent::qq_chat::ignore_store::{
    create_ignore_rule, delete_ignore_rule, list_ignore_rules, update_ignore_rule, QqChatAgentServiceIgnoreRuleUpsert,
};
use crate::agent::qq_chat::privilege_store::{delete_all_notifications, list_recent_notifications};
...
use zihuan_core::model_inference::system_config::{AgentConfig, AgentToolConfig, AgentType, LlmRefConfig};
...
pub fn build_agent_task_runtime(state: Arc<AppState>, broadcast_tx: WsBroadcast) -> Arc<dyn AgentTaskRuntime> {
    if let Some(existing) = crate::command::global_task_runtime() {
        return existing;
    }
    ...
    crate::command::set_global_task_runtime(Arc::clone(&runtime));
    runtime
}
...
async fn resolve_qq_chat_profile(
    ...
    if let Some(user_id) = profile.bot_user_id.as_deref() {
        if let Err(err) = persist_qq_avatar_for_agent(agent, user_id).await {
            warn!(
                "[agents] failed to persist QQ avatar for agent '{}' (connection='{}'): {}",
                agent.id, config.ims_bot_adapter_connection_id, err
            );
        }
    }
    ...
    Some(QqChatProfile {
        ...
        bot_avatar_url: profile.avatar_url,
    })
}
...
async fn resolve_agent_rdb_connection(agent_id: &str) -> CoreResult<zihuan_core::data_refs::RelationalDbConnection> {
    ...
    zihuan_core::storage_handler::build_relational_db_connection_for_connection(rdb_id, &connections).await
}
...
fn validate_qq_chat_agent_service_llms(
    ...
    match agent_type {
        AgentType::QqChat(config) => {
            ...
            match &llm_ref.model {
                zihuan_core::model_inference::system_config::ModelRefSpec::ChatLlm { llm } => {
                ...
                zihuan_core::model_inference::system_config::ModelRefSpec::TextEmbeddingLocal { .. } => Err(format!(
...
fn validate_chat_llm_ref(
    ...
    match llm_ref.model {
        zihuan_core::model_inference::system_config::ModelRefSpec::ChatLlm { .. } => Ok(()),
        zihuan_core::model_inference::system_config::ModelRefSpec::TextEmbeddingLocal { .. } => Err(format!(
...
fn validate_embedding_model_ref(
    ...
    match llm_ref.model {
        zihuan_core::model_inference::system_config::ModelRefSpec::TextEmbeddingLocal { .. } => Ok(()),
        zihuan_core::model_inference::system_config::ModelRefSpec::ChatLlm { .. } => Err(format!(
...
pub async fn upload_avatar(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let store = match avatar_store().await {
        Ok(store) => store,
        Err(err) => {
            res.status_code(StatusCode::INTERNAL_SERVER_ERROR);
            res.render(format!("Failed to save avatar: {}", err));
            return;
        }
    };
...
pub async fn get_avatar(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let store = match avatar_store().await {
        ...
        Err(err) => {
            res.status_code(StatusCode::INTERNAL_SERVER_ERROR);
            res.render(format!("Failed to load avatar: {}", err));
            return;
        }
    };

    match store.get_avatar(&avatar_id).await.map_err(|e| e.to_string()) {
        Ok(Some(avatar)) => {
...
async fn delete_avatar_by_agent_id(agent_id: &str) -> Result<(), String> {
    avatar_store()
        .await?
        .delete_avatar_by_agent(agent_id)
        .await
        .map_err(|e| e.to_string())
}
...
async fn persist_qq_avatar_for_agent(agent: &mut AgentConfig, user_id: &str) -> Result<(), String> {
    ...
    if !response.status().is_success() {
        return Err(format!("Failed to download QQ avatar: HTTP {}", response.status()));
    }
    ...
    let image_data = response
        .bytes()
        .await
        .map_err(|e| format!("Failed to read QQ avatar bytes: {}", e))?;
    let avatar_id = avatar_store()
        .await?
        .save_avatar(&agent.id, None, &mime_type, image_data.to_vec())
        .await
        .map_err(|e| e.to_string())?;

    let persisted_avatar_url = format!("avatar://{}", avatar_id);
    if agent.avatar_url.as_deref() == Some(persisted_avatar_url.as_str()) {
        return Ok(());
    }
...
async fn avatar_store() -> Result<Arc<dyn AgentAvatarStore>, String> {
    first_available_agent_avatar_store().await.map_err(|e| e.to_string())
}
