...
fn model_http_settings_response(
    settings: ModelHttpServiceSettings,
    endpoint: String,
...
pub async fn get_model_http_settings(_req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    match zihuan_core::system_config::load_section::<GlobalSettingsSection>() {
        Ok(settings) => {
            res.render(Json(model_http_settings_response(settings.model_http_service, endpoint)))
        }
        Err(error) => {
...
pub async fn update_model_http_settings(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let body: UpdateModelHttpSettingsRequest = match req.parse_json().await {
        Ok(body) => body,
        Err(error) => {
            return render_settings_error(res, StatusCode::BAD_REQUEST, error.to_string())
        }
    };
    let llm_refs = match load_llm_refs() {
        Ok(items) => items,
        Err(error) => {
            return render_settings_error(res, StatusCode::INTERNAL_SERVER_ERROR, error.to_string())
        }
    };
    let mut public_model_names = std::collections::HashSet::new();
    for config_id in &body.public_model_config_ids {
        let Some(llm_ref) = llm_refs.iter().find(|item| item.id == *config_id && item.enabled)
        else {
            return render_settings_error(
                res,
                StatusCode::BAD_REQUEST,
                format!("public model '{config_id}' is not an enabled chat model"),
            );
        };
        let ModelRefSpec::ChatLlm { llm } = &llm_ref.model else {
            return render_settings_error(
                res,
                StatusCode::BAD_REQUEST,
                format!("public model '{}' is not a chat model", llm_ref.name),
            );
        };
        if !public_model_names.insert(llm_ref.name.clone()) {
            return render_settings_error(
                res,
                StatusCode::BAD_REQUEST,
                format!("public models must use unique configuration names: {}", llm_ref.name),
            );
        }
    }
...
pub async fn create_model_http_api_key(req: &mut Request, res: &mut Response) {
    ...
    match zihuan_core::system_config::save_section::<GlobalSettingsSection>(&settings) {
        Ok(()) => res.render(Json(CreateModelHttpApiKeyResponse {
            key: model_http_key_response(&key),
            secret,
        })),
        Err(error) => {
            render_settings_error(res, StatusCode::INTERNAL_SERVER_ERROR, error.to_string())
        ...
        Ok(entries) => {
            let mut models: Vec<ModelEntry> = entries
                ...
                .filter_map(|entry| {
                    ...
                    Some(ModelEntry { name, path: abs, valid, size_bytes })
                })
                .collect();
