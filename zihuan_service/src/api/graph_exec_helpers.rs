...
use zihuan_core::graph_engine::data_value::DataType;
use zihuan_core::graph_engine::function_graph::{embedded_function_config_from_node, FUNCTION_CONFIG_PORT};
use zihuan_core::graph_engine::graph_io::{NodeGraphDefinition, PortBindingKind};
use zihuan_core::graph_engine::{DataValue, NodeGraph};
    ...
    for node in &mut graph.nodes {
        ...
        if let Some(mut config) = embedded_function_config_from_node(node) {
            ...
            if let Ok(value) = serde_json::to_value(&config) {
                node.inline_values.insert(FUNCTION_CONFIG_PORT.to_string(), value);
            }
        }

        if let Some(tools_value) = node.inline_values.get("tools_config").cloned() {
            if let Ok(mut tools) =
                serde_json::from_value::<Vec<zihuan_core::graph_engine::brain_tool_spec::BrainToolDefinition>>(tools_value)
            {
