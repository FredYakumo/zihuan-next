...
pub async fn chat_completions(req: &mut Request, res: &mut Response) {
    ...
    let Some((config, _)) =
        public_models(&settings.model_http_service.public_model_config_ids, &llm_refs)
            .into_iter()
            .find(|(_, model_name)| *model_name == body.model)
    else {
        return render_error(
            res,
    ...
    let ModelRefSpec::ChatLlm { llm } = &config.model else {
        unreachable!()
    };
    ...
    let messages = match parse_openai_messages(body.messages) {
        ...
        Err(message) => {
            return render_error(res, StatusCode::BAD_REQUEST, "invalid_request_error", &message)
        }
    };
    let tools = body.tools.as_ref().map(|items| {
        items
            .iter()
            .filter(|item| item.type_name == "function")
            .map(|item| {
                    ...
                    .filter_map(|call| {
                        Some(zihuan_core::model_inference::llm::tooling::ToolCalls {
                            ...
                            type_name: call
                                ...
                                .to_string(),
                            function:
                                zihuan_core::model_inference::llm::tooling::ToolCallsFuncSpec {
...
fn authorize(req: &Request) -> bool {
    ...
    if !settings.model_http_service.enabled {
        return false;
    }
    ...
    settings
        ...
        .api_keys
        .iter()
        .any(|key| api_key_is_valid(key, &hash))
}
...
fn public_models<'a>(
    ids: &[String],
    models: &'a [LlmRefConfig],
) -> Vec<(&'a LlmRefConfig, String)> {
    models
        .iter()
        .filter_map(|item| {
            if !item.enabled || !ids.iter().any(|id| id == &item.id) {
                return None;
            }
            match &item.model {
...
fn build_sse_response(id: &str, created: i64, model: &str, message: &LLMMessage) -> String {
    let mut chunks = vec![
        serde_json::json!({ "id": id, "object": "chat.completion.chunk", "created": created, "model": model, "choices": [{ "index": 0, "delta": { "role": "assistant" }, "finish_reason": serde_json::Value::Null }] }),
    ];
    if !message.tool_calls.is_empty() {
    ...
    let mut response =
        chunks.into_iter().map(|chunk| format!("data: {chunk}\n\n")).collect::<String>();
