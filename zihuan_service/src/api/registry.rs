...
use zihuan_core::graph::registry::NODE_REGISTRY;
...
pub async fn get_registry(_req: &mut Request, res: &mut Response, _depot: &mut Depot) {
    ...
    let mut types: Vec<NodeTypeInfo> = all_types
        ...
        .map(|meta| {
            let (input_ports, output_ports) =
                NODE_REGISTRY.get_node_ports(&meta.type_id).unwrap_or_default();

            let (has_dyn_in, has_dyn_out) = NODE_REGISTRY
                .get_node_dynamic_port_flags(&meta.type_id)
                .unwrap_or((false, false));
            let config_fields =
                NODE_REGISTRY.get_node_config_fields(&meta.type_id).unwrap_or_default();
    ...
    types.sort_by(|a, b| {
        a.category.cmp(&b.category).then_with(|| a.display_name.cmp(&b.display_name))
    });
