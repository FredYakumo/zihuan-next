...
async fn connection_for_service(
    service_id: &str,
) -> Result<zihuan_core::data_refs::RelationalDbConnection, String> {
    ...
    zihuan_core::storage::build_relational_db_connection_for_connection(rdb_id, &connections)
        .await
...
pub async fn list_scheduled_tasks(req: &mut Request, res: &mut Response, _depot: &mut Depot) {
    ...
    let Some(service_id) = service_id else {
        return render_bad_request(res, "service_id is required".to_string());
    };
...
pub async fn cancel_scheduled_task(req: &mut Request, res: &mut Response, _depot: &mut Depot) {
    ...
    let result = async {
        let connection = connection_for_service(&service_id).await?;
        let tasks = zihuan_core::scheduled_task::list_tasks(&connection, Some(&service_id), None)
            .await
            .map_err(|err| err.to_string())?;
        let Some(task) = tasks.into_iter().find(|task| task.id == task_id) else {
            return Err("计划任务不存在".to_string());
        };
        if task.status != zihuan_core::scheduled_task::ScheduledTaskStatus::Pending {
            return Err("只有等待中的计划任务可以取消".to_string());
        }
        zihuan_core::scheduled_task::finish_task(
            &connection,
            &task_id,
            zihuan_core::scheduled_task::ScheduledTaskStatus::Cancelled,
            Some("已由管理员取消"),
        )
        .await
        .map_err(|err| err.to_string())
