...
fn validate_duplicate_port_names(graph: &zihuan_core::graph_engine::graph_io::NodeGraphDefinition) -> Result<(), String> {
...
fn run_graph_blocking(
    definition: zihuan_core::graph_engine::graph_io::NodeGraphDefinition,
    ...
    let mut graph = zihuan_core::graph_engine::registry::build_node_graph_from_definition(&definition)
...
pub async fn rerun_task(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let (file_path, user_ip) = {
        ...
        let Some(file_path) = task.file_path.clone() else {
            res.status_code(StatusCode::BAD_REQUEST);
            res.render(Json(
                serde_json::json!({"error": "This task cannot be rerun because it has no saved file path"}),
            ));
            return;
        };
    ...
    let mut graph = match zihuan_core::graph_engine::load_graph_definition_from_json(&file_path) {
        Ok(graph) => graph,
        Err(e) => {
...
fn start_graph_task(
    state: Arc<AppState>,
    broadcast_tx: WsBroadcast,
    graph_def: zihuan_core::graph_engine::graph_io::NodeGraphDefinition,
    graph_name: String,
    graph_session_id: String,
