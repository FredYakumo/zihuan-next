...
fn run_graph_blocking(
    definition: zihuan_core::graph::graph_io::NodeGraphDefinition,
...
pub async fn rerun_task(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let mut graph = match zihuan_core::graph::load_graph_definition_from_json(&file_path) {
        Ok(graph) => graph,
        Err(e) => {
...
fn start_graph_task(
    state: Arc<AppState>,
    broadcast_tx: WsBroadcast,
    graph_def: zihuan_core::graph::graph_io::NodeGraphDefinition,
    graph_name: String,
    graph_session_id: String,
