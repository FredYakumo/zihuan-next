...
pub async fn add_node(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let (input_ports, output_ports) = zihuan_core::graph::registry::NODE_REGISTRY
        .get_node_ports(&body.node_type)
        .unwrap_or_default();

    let (dyn_in, dyn_out) = zihuan_core::graph::registry::NODE_REGISTRY
...
pub async fn add_edge(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    session.graph.edges.push(zihuan_core::graph::graph_io::EdgeDefinition {
...
pub async fn validate_graph(req: &mut Request, res: &mut Response, depot: &mut Depot) {
    ...
    let issues = zihuan_core::graph::graph_io::validate_graph_definition(&session.graph);
    let cycle_nodes = zihuan_core::graph::graph_io::find_cycle_node_ids(&session.graph);
