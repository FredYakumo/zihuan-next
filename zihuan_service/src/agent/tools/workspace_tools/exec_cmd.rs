...
impl BrainTool for ExecCmdBrainTool{
 ...
 fn execute_with_progress(&self, _:&str, a:&Value, on_output: Arc<dyn Fn(&str, &str) + Send + Sync>)->zihuan_agent::brain::ToolExecutionOutput{
      let args:ExecCmdArgs=match serde_json::from_value(a.clone()){Ok(v)=>v,Err(e)=>return zihuan_agent::brain::ToolExecutionOutput::text(json_error(format!("invalid exec_cmd arguments: {e}")))};
  let shell=args.shell.clone().unwrap_or_else(||if cfg!(windows){"powershell".to_string()}else{"bash".to_string()});
      if shell!="powershell"&&shell!="bash"{return zihuan_agent::brain::ToolExecutionOutput::text(json_error("shell must be powershell or bash"));}
      let cwd=if let Some(raw)=args.cwd.as_deref(){match resolve_tool_path(self.workspace_path.as_deref(),raw){Ok(v)=>Some(v),Err(e)=>return zihuan_agent::brain::ToolExecutionOutput::text(json_error(e.to_string()))}}else{self.workspace_path.clone()};
      let secs=args.timeout_secs.unwrap_or(30);let max_output=args.max_output_bytes.unwrap_or(32*1024);if max_output==0{return zihuan_agent::brain::ToolExecutionOutput::text(json_error("max_output_bytes must be greater than zero"));}
  let command_cwd=cwd.clone(); let input=args.input.clone(); let env=args.env.clone(); let command_text=args.command.clone(); let selected_shell=shell.clone();
   ...
   if let Some(input)=input{use tokio::io::AsyncWriteExt;if let Some(mut stdin)=child.stdin.take(){let _=stdin.write_all(input.as_bytes()).await;}}
      ...
      zihuan_agent::brain::ToolExecutionOutput::text(result)
 }
