...
async fn run() -> Result<()> {
    ...
    let graph_def = zihuan_core::graph_engine::load_graph_definition_from_json(&graph_path)?;
    let mut graph = zihuan_core::graph_engine::build_node_graph_from_definition(&graph_def)?;
    graph.execute()?;
...
fn init_node_registry() -> Result<()> {
    zihuan_core::graph_engine::registry::init_node_registry_with_extensions(&[
        ...
        zihuan_ims_agent::init_node_registry,
        zihuan_workspace_agent::init_node_registry,
    ])
}
