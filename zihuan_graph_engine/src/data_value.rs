...
impl TavilyRef {
    ...
    pub fn extract_url(&self, url: &str) -> zihuan_core::error::Result<Vec<String>> {
        let client = Client::builder().timeout(self.timeout).build()?;
        let response = client
    ...
    pub fn fetch_url_direct(&self, url: &str) -> zihuan_core::error::Result<Vec<String>> {
        let client = Client::builder().timeout(self.timeout).build()?;
        let response = client
            .get(url)
            .header(
...
///
/// This reference points at the live storage owned by the service-layer agent or provider node.
...
pub struct ImageData {
    pub metadata: ImageMessage,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub binary: Option<Vec<u8>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub object_storage_path: Option<String>,
}
