...
- **Never use ASCII-art separator comments** (`// ----`, `// ====`, `/* ==== */`, etc.) to delineate sections within a file. When a logical section boundary needs documentation, write a detailed doc comment (`///`) that covers: **Purpose** (what this section does and why it exists), **Design** (the high-level approach, key design decisions, and trade-offs), and **Architecture** (how it fits with the rest of the system — use Markdown-style descriptions, never ASCII diagrams or flowcharts). For brief inline comments within a function body, keep only the **Purpose** — a short explanation of what the following code block does and why it exists.
