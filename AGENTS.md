...
**Unified LLM Message Model** — Internal conversation/state/history flow uses `zihuan_core::llm::LLMMessage` as the project-wide message type. `LLMMessage` contains `role`, `parts`, `tool_calls`, `tool_call_id`, `reasoning_content`, and `usage`. Multimodal parts use `LLMMessagePart`, and image/video parts must carry the existing project media model (`PersistedMedia`) rather than raw provider-specific URL structs. Provider-specific request/response shapes belong in convert modules, not in the core message type.
...
- **LLM message naming.** Use `LLMMessage`, `LLMMessagePart`, and `LLMMessageSessionCacheRef` in new Rust code. Do not introduce new `OpenAIMessage*` names for internal message flow.
